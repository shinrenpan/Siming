    ...
    group.post { request, _ in
        ...
        if let ifNoneExist = request.headers[goalIfNoneExistHeader] {
            ...
            if let existing = matches.entries.first {
                ...
                headers[.location] = "\(serverBaseURL(request))/Goal/\(existing.id)/_history/\(existing.versionId)"
        ...
        headers[.location] = "\(serverBaseURL(request))/Goal/\(result.id)/_history/\(result.versionId)"
