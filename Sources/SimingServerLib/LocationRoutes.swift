...
func parseLocationQuery(from pairs: some Collection<(key: Substring, value: Substring)>) -> LocationSearchQuery {
    ...
    let sort        = LocationSearchQuery.SortOrder.parse(first("_sort").map(String.init) ?? "-_lastUpdated")
    let count       = min(first("_count").flatMap { Int($0) } ?? 20, maxCount)
    let cursor      = first("_cursor").flatMap { LocationSearchQuery.SearchCursor.decode(String($0)) }
...
private func nextLocationPageURL(selfURL: String, cursor: LocationSearchQuery.SearchCursor, count: Int) -> String {
