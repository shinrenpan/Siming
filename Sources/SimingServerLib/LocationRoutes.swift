...
public func addLocationRoutes(
    ...
    let group = router.group("Location")
    ...
    group.post { request, _ in
        ...
        if let ifNoneExist = request.headers[ifNoneExistHeader] {
            ...
            var checkQuery = try parseLocationQuery(from: pairs)
            ...
            if let existing = matches.entries.first {
                ...
                headers[.location] = "\(serverBaseURL(request))/Location/\(existing.id)/_history/\(existing.versionId)"
                return Response(status: .ok, headers: headers,
                                body: preferBody(preferReturn, resource: existing.jsonWithMeta))
            }
        }

        let result = try await store.create(loc)
        var headers = HTTPFields()
        headers[.contentType]  = fhirJSON
        headers[.eTag]         = "W/\"\(result.versionId)\""
        headers[.lastModified] = httpDate(result.lastUpdated)
        headers[.location] = "\(serverBaseURL(request))/Location/\(result.id)/_history/\(result.versionId)"
    ...
    group.put { request, _ in
        ...
        var checkQuery = try parseLocationQuery(from: qpPairs)
        checkQuery.count = 2; checkQuery.totalMode = .none; checkQuery.cursor = nil
        let matches = try await store.search(query: checkQuery)
    ...
    // GET /Location/:id/_history/:vid — vread
    group.get(":id/_history/:vid") { request, context in
        ...
        if let s = parseSummary(from: qpPairs), s != .false && s != .count {
            jsonData = applySummary(jsonData, mode: s, summaryFields: locationSummaryFields)
    ...
    // GET /Location/_history — type-level history
    ...
    // GET /Location/:id/_history — instance history
    ...
    group.patch(":id") { request, context in
        ...
        let loc: Location
        do { loc = try JSONDecoder().decode(Location.self, from: patchedJSON) }
    ...
    // DELETE /Location?<search> — conditional delete
    group.delete { request, _ in
        let qpPairs = request.uri.queryParameters.map { (key: $0.key, value: $0.value) }
        guard !qpPairs.isEmpty else {
            throw FHIRRouteError.invalidBody("DELETE /Location requires search parameters for conditional delete")
        }
        var checkQuery = try parseLocationQuery(from: qpPairs)
        checkQuery.count = 2; checkQuery.totalMode = .none; checkQuery.cursor = nil
        let matches = try await store.search(query: checkQuery)
        switch matches.entries.count {
        case 0:
            throw FHIRServerError.notFound(resourceType: "Location", id: "(search)")
        case 1:
            let ifMatch = parseETag(request.headers[.ifMatch])
    ...
    // DELETE /Location/:id — logical delete
    ...
    group.get { request, _ in
        ...
        if isStrictHandling(request) {
            let bad = unknownParams(in: pairs, known: knownLocationParams)
            if !bad.isEmpty { throw FHIRRouteError.unknownParams(bad) }
        }
        var query = try parseLocationQuery(from: pairs)
        ...
        let nextURL = result.nextCursor.map { nextLocationPageURL(selfURL: base, cursor: $0, count: query.count) }
        let entries = result.entries.map { e -> (fullUrl: String, json: Data) in
            var json = e.jsonWithMeta
            if let s = summary, s != .false { json = applySummary(json, mode: s, summaryFields: locationSummaryFields) }
            if let elems = elements { json = applyElements(json, elements: elems) }
            return ("\(baseURL)/Location/\(e.id)", json)
        }
    ...
    // POST /Location/_search — form-encoded search
...
func parseLocationQuery(from pairs: some Collection<(key: Substring, value: Substring)>) throws -> LocationSearchQuery {
    ...
    let name             = LocationSearchQuery.StringParam.parse(key: "name",    from: pairs)
    let address          = LocationSearchQuery.StringParam.parse(key: "address", from: pairs)
    let addressCity      = LocationSearchQuery.StringParam.parse(key: "address-city",        from: pairs)
    let addressState     = LocationSearchQuery.StringParam.parse(key: "address-state",       from: pairs)
    let addressPostalCode = LocationSearchQuery.StringParam.parse(key: "address-postalcode", from: pairs)
    let addressCountry   = LocationSearchQuery.StringParam.parse(key: "address-country",     from: pairs)

    let status           = all("status").flatMap { LocationSearchQuery.TokenParam.parseList(String($0)) }
    ...
    let lastUpdated = try parseDateParams(all("_lastUpdated"), "_lastUpdated", LocationSearchQuery.DateParam.parse)
    let sortKeys = LocationSearchQuery.parseSortKeys(first("_sort").map(String.init) ?? "-_lastUpdated")
...
private func nextLocationPageURL(selfURL: String, cursor: SearchCursor, count: Int) -> String {
