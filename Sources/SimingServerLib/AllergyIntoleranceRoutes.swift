...
public func addAllergyIntoleranceRoutes(
    ...
    group.post { request, _ in
        ...
        if let ifNoneExist = request.headers[ifNoneExistHeader] {
            ...
            if let existing = matches.entries.first {
                ...
                headers[.location] = "\(serverBaseURL(request))/AllergyIntolerance/\(existing.id)/_history/\(existing.versionId)"
        ...
        headers[.location] = "\(serverBaseURL(request))/AllergyIntolerance/\(result.id)/_history/\(result.versionId)"
    ...
    // PATCH /AllergyIntolerance/:id — JSON Patch (RFC 6902)
    group.patch(":id") { request, context in
        ...
        let ai: AllergyIntolerance
        do { ai = try JSONDecoder().decode(AllergyIntolerance.self, from: patchedJSON) }
