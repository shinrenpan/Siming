    ...
    group.post { request, _ in
        ...
        if let ifNoneExist = request.headers[fmhIfNoneExistHeader] {
            ...
            if let existing = matches.entries.first {
                ...
                headers[.location] = "\(serverBaseURL(request))/FamilyMemberHistory/\(existing.id)/_history/\(existing.versionId)"
        ...
        headers[.location] = "\(serverBaseURL(request))/FamilyMemberHistory/\(result.id)/_history/\(result.versionId)"
