...
// Only `practitioner` is indexed so far — the remaining R4 params are visible as
// TODO markers in Generated/PractitionerRole+SearchExtractor.swift.
let knownPractitionerRoleParams: Set<String> = [
    "practitioner", "practitioner:missing",
...
public func addPractitionerRoleRoutes(
    ...
    let group = router.group("PractitionerRole")

    // POST /PractitionerRole — create
    group.post { request, _ in
        ...
        try validateResourceType("PractitionerRole", from: Data(bodyBuffer.readableBytesView))
        ...
        if let ifNoneExist = request.headers[ifNoneExistHeader] {
            ...
            if matches.entries.count > 1 {
                throw FHIRServerError.multipleMatches(resourceType: "PractitionerRole")
            }
            if let existing = matches.entries.first {
                ...
                headers[.location] = "\(serverBaseURL(request))/PractitionerRole/\(existing.id)/_history/\(existing.versionId)"
                return Response(status: .ok, headers: headers,
                                body: preferBody(preferReturn, resource: existing.jsonWithMeta))
            }
        }

        let result = try await store.create(role)
        var headers = HTTPFields()
        headers[.contentType]  = fhirJSON
        headers[.eTag]         = "W/\"\(result.versionId)\""
        headers[.lastModified] = httpDate(result.lastUpdated)
        headers[.location] = "\(serverBaseURL(request))/PractitionerRole/\(result.id)/_history/\(result.versionId)"
        return Response(status: .created, headers: headers,
                        body: preferBody(preferReturn, resource: result.jsonData))
    }

    // PUT /PractitionerRole?<search> — conditional update
    group.put { request, _ in
        try requireFHIRContentType(request)
        let preferReturn = parsePreferReturn(request.headers[preferHeader])
        let qpPairs = request.uri.queryParameters.map { (key: $0.key, value: $0.value) }
        guard !qpPairs.isEmpty else {
            throw FHIRRouteError.invalidBody("PUT /PractitionerRole requires search parameters for conditional update")
        }
        var req = request
        let bodyBuffer = try await req.collectBody(upTo: maxBodyBytes)
        try validateResourceType("PractitionerRole", from: Data(bodyBuffer.readableBytesView))
        ...
        case 1:
            ...
            let result = try await store.update(id: existingId, role: role, ifMatch: ifMatch)
            var headers = HTTPFields()
            headers[.contentType]  = fhirJSON
            headers[.eTag]         = "W/\"\(result.versionId)\""
            headers[.lastModified] = httpDate(result.lastUpdated)
    ...
    group.get(":id") { request, context in
        ...
        if let s = summary, s != .false { json = applySummary(json, mode: s, summaryFields: practitionerRoleSummaryFields) }
    ...
    // GET /PractitionerRole/:id/_history/:vid — vread
    ...
    // GET /PractitionerRole/:id/_history — instance history
    ...
    // GET /PractitionerRole/_history — type history
    ...
    // PUT /PractitionerRole/:id — update
    group.put(":id") { request, context in
        ...
        try validateResourceType("PractitionerRole", from: Data(bodyBuffer.readableBytesView))
        let role = try decodeFHIR(PractitionerRole.self, from: bodyBuffer)
        let result = try await store.update(id: id, role: role, ifMatch: ifMatch)
        var headers = HTTPFields()
        headers[.contentType]  = fhirJSON
        headers[.eTag]         = "W/\"\(result.versionId)\""
        headers[.lastModified] = httpDate(result.lastUpdated)
    ...
    // PATCH /PractitionerRole/:id — JSON Patch (RFC 6902)
    group.patch(":id") { request, context in
        ...
        do { role = try JSONDecoder().decode(PractitionerRole.self, from: patchedJSON) }
    ...
    // DELETE /PractitionerRole/:id — logical delete
    ...
    // DELETE /PractitionerRole?<search> — conditional delete
    ...
    group.get { request, _ in
        ...
        let nextURL = result.nextCursor.map { nextPractitionerRolePageURL(selfURL: base, cursor: $0, count: query.count) }
        let entries = result.entries.map { e -> (fullUrl: String, json: Data) in
            var json = e.jsonWithMeta
            if let s = summary, s != .false { json = applySummary(json, mode: s, summaryFields: practitionerRoleSummaryFields) }
            if let elems = elements { json = applyElements(json, elements: elems) }
            return ("\(baseURL)/PractitionerRole/\(e.id)", json)
        }
    ...
    // POST /PractitionerRole/_search — form-encoded search
...
func parsePractitionerRoleQuery(from pairs: some Collection<(key: Substring, value: Substring)>) -> PractitionerRoleSearchQuery {
    ...
    let totalMode = PractitionerRoleSearchQuery.TotalMode.parse(first("_total").map(String.init))
    ...
    var query = PractitionerRoleSearchQuery(
...
private func nextPractitionerRolePageURL(selfURL: String, cursor: SearchCursor, count: Int) -> String {
