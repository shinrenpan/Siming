...
public func addMedicationRoutes(
    ...
    group.post { request, _ in
        ...
        if let ifNoneExist = request.headers[ifNoneExistHeader] {
            ...
            if let existing = matches.entries.first {
                ...
                headers[.location]     = "/Medication/\(existing.id)/_history/\(existing.versionId)"
        ...
        headers[.location]     = "/Medication/\(result.id)/_history/\(result.versionId)"
    ...
    group.get { request, _ in
        ...
        let query = parseMedicationQuery(from: pairs)
...
func parseMedicationQuery(from pairs: some Collection<(key: Substring, value: Substring)>) -> MedicationSearchQuery {
    ...
    let sort        = MedicationSearchQuery.SortOrder.parse(first("_sort").map(String.init) ?? "-_lastUpdated")
    let count       = min(first("_count").flatMap { Int($0) } ?? 20, maxCount)
    let cursor      = first("_cursor").flatMap { MedicationSearchQuery.SearchCursor.decode(String($0)) }
...
private func nextMedicationPageURL(selfURL: String, cursor: MedicationSearchQuery.SearchCursor, count: Int) -> String {
