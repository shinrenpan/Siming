...
public func addOrganizationRoutes(
    ...
    let group = router.group("Organization")
    ...
    group.post { request, _ in
        ...
        if let ifNoneExist = request.headers[ifNoneExistHeader] {
            ...
            var checkQuery = try parseOrganizationQuery(from: pairs)
            ...
            if matches.entries.count > 1 {
                throw FHIRServerError.multipleMatches(resourceType: "Organization")
            }
    ...
    group.put { request, _ in
        ...
        var checkQuery = try parseOrganizationQuery(from: qpPairs)
        checkQuery.count = 2; checkQuery.totalMode = .none; checkQuery.cursor = nil
        let matches = try await store.search(query: checkQuery)
        ...
        case 1:
            ...
            let result = try await store.update(id: existingId, organization: org, ifMatch: ifMatch)
            var headers = HTTPFields()
            headers[.contentType]  = fhirJSON
            headers[.eTag]         = "W/\"\(result.versionId)\""
            headers[.lastModified] = httpDate(result.lastUpdated)
    ...
    // GET /Organization/:id/_history/:vid — vread
    group.get(":id/_history/:vid") { request, context in
        ...
        if let s = parseSummary(from: qpPairs), s != .false && s != .count {
            jsonData = applySummary(jsonData, mode: s, summaryFields: organizationSummaryFields)
    ...
    // GET /Organization/_history — type-level history
    ...
    // GET /Organization/:id/_history — instance history
    ...
    // PATCH /Organization/:id — JSON Patch (RFC 6902)
    group.patch(":id") { request, context in
        ...
        do { org = try JSONDecoder().decode(Organization.self, from: patchedJSON) }
    ...
    // DELETE /Organization?<search> — conditional delete
    group.delete { request, _ in
        let qpPairs = request.uri.queryParameters.map { (key: $0.key, value: $0.value) }
        guard !qpPairs.isEmpty else {
            throw FHIRRouteError.invalidBody("DELETE /Organization requires search parameters for conditional delete")
        }
        var checkQuery = try parseOrganizationQuery(from: qpPairs)
        checkQuery.count = 2; checkQuery.totalMode = .none; checkQuery.cursor = nil
        let matches = try await store.search(query: checkQuery)
        switch matches.entries.count {
        case 0:
            throw FHIRServerError.notFound(resourceType: "Organization", id: "(search)")
        case 1:
            let ifMatch = parseETag(request.headers[.ifMatch])
    ...
    // DELETE /Organization/:id — logical delete
    ...
    group.get { request, _ in
        ...
        if isStrictHandling(request) {
            let bad = unknownParams(in: pairs, known: knownOrganizationParams)
            if !bad.isEmpty { throw FHIRRouteError.unknownParams(bad) }
        }
        var query = try parseOrganizationQuery(from: pairs)
        ...
        let nextURL = result.nextCursor.map { nextOrganizationPageURL(selfURL: base, cursor: $0, count: query.count) }
        let entries = result.entries.map { e -> (fullUrl: String, json: Data) in
            var json = e.jsonWithMeta
            if let s = summary, s != .false { json = applySummary(json, mode: s, summaryFields: organizationSummaryFields) }
            if let elems = elements { json = applyElements(json, elements: elems) }
            return ("\(baseURL)/Organization/\(e.id)", json)
        }
    ...
    // POST /Organization/_search — form-encoded search
...
func parseOrganizationQuery(from pairs: some Collection<(key: Substring, value: Substring)>) throws -> OrganizationSearchQuery {
    ...
    let addressCountry = OrganizationSearchQuery.StringParam.parse(key: "address-country",     from: pairs)
    ...
    let lastUpdated = try parseDateParams(all("_lastUpdated"), "_lastUpdated", OrganizationSearchQuery.DateParam.parse)
    let sortKeys = OrganizationSearchQuery.parseSortKeys(first("_sort").map(String.init) ?? "-_lastUpdated")
...
private func nextOrganizationPageURL(selfURL: String, cursor: SearchCursor, count: Int) -> String {
