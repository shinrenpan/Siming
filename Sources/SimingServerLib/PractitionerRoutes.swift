...
public func addPractitionerRoutes(
    ...
    group.post { request, _ in
        ...
        if let ifNoneExist = request.headers[ifNoneExistHeader] {
            ...
            if let existing = matches.entries.first {
                ...
                headers[.location]     = "/Practitioner/\(existing.id)/_history/\(existing.versionId)"
        ...
        headers[.location]     = "/Practitioner/\(result.id)/_history/\(result.versionId)"
    ...
    group.get { request, _ in
        ...
        let query = parsePractitionerQuery(from: pairs)
...
func parsePractitionerQuery(from pairs: some Collection<(key: Substring, value: Substring)>) -> PractitionerSearchQuery {
    ...
    let sort        = PractitionerSearchQuery.SortOrder.parse(first("_sort").map(String.init) ?? "-_lastUpdated")
    let count       = min(first("_count").flatMap { Int($0) } ?? 20, maxCount)
    let cursor      = first("_cursor").flatMap { PractitionerSearchQuery.SearchCursor.decode(String($0)) }
...
private func nextPractitionerPageURL(selfURL: String, cursor: PractitionerSearchQuery.SearchCursor, count: Int) -> String {
