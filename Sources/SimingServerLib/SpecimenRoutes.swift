...
func parseSpecimenQuery(from pairs: some Collection<(key: Substring, value: Substring)>) -> SpecimenSearchQuery {
    ...
    let sort        = SpecimenSearchQuery.SortOrder.parse(first("_sort").map(String.init) ?? "-_lastUpdated")
    let count       = min(first("_count").flatMap { Int($0) } ?? 20, specMaxCount)
    let cursor      = first("_cursor").flatMap { SpecimenSearchQuery.SearchCursor.decode(String($0)) }
...
func nextSpecimenPageURL(selfURL: String, cursor: SpecimenSearchQuery.SearchCursor, count: Int) -> String {
