...
public func addProcedureRoutes(
    ...
    group.get { request, _ in
        ...
        let query = parseProcedureQuery(from: pairs)
...
func parseProcedureQuery(from pairs: some Collection<(key: Substring, value: Substring)>) -> ProcedureSearchQuery {
    ...
    let sort        = ProcedureSearchQuery.SortOrder.parse(first("_sort").map(String.init) ?? "-_lastUpdated")
...
private func nextProcedurePageURL(selfURL: String, cursor: ProcedureSearchQuery.SearchCursor, count: Int) -> String {
