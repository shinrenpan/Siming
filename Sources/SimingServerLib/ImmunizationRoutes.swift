...
public func addImmunizationRoutes(
    to router: Router<BasicRequestContext>,
    store: ImmunizationStore,
    logger: Logger
) {
    let group = router.group("Immunization")
    ...
    group.post { request, _ in
        ...
        try validateResourceType("Immunization", from: Data(bodyBuffer.readableBytesView))
        ...
        if let ifNoneExist = request.headers[ifNoneExistHeader] {
            ...
            if matches.entries.count > 1 {
                throw FHIRServerError.multipleMatches(resourceType: "Immunization")
            }
    ...
    group.put { request, _ in
        ...
        case 1:
            ...
            let result = try await store.update(id: existingId, immunization: imm, ifMatch: ifMatch)
            var headers = HTTPFields()
            headers[.contentType]  = fhirJSON
            headers[.eTag]         = "W/\"\(result.versionId)\""
            headers[.lastModified] = httpDate(result.lastUpdated)
    ...
    // GET /Immunization/:id/_history/:vid — vread
    group.get(":id/_history/:vid") { request, context in
        ...
        if let s = parseSummary(from: qpPairs), s != .false && s != .count {
            jsonData = applySummary(jsonData, mode: s, summaryFields: immunizationSummaryFields)
    ...
    // GET /Immunization/_history — type-level history
    ...
    // GET /Immunization/:id/_history — instance history
    ...
    group.put(":id") { request, context in
        ...
        try validateResourceType("Immunization", from: Data(bodyBuffer.readableBytesView))
        let imm = try decodeFHIR(Immunization.self, from: bodyBuffer)
        let result = try await store.update(id: id, immunization: imm, ifMatch: ifMatch)
        var headers = HTTPFields()
        headers[.contentType]  = fhirJSON
        headers[.eTag]         = "W/\"\(result.versionId)\""
        headers[.lastModified] = httpDate(result.lastUpdated)
    ...
    // PATCH /Immunization/:id — JSON Patch (RFC 6902)
    group.patch(":id") { request, context in
        ...
        do { imm = try JSONDecoder().decode(Immunization.self, from: patchedJSON) }
    ...
    // DELETE /Immunization?<search> — conditional delete
    group.delete { request, _ in
        ...
        switch matches.entries.count {
        case 0:
            throw FHIRServerError.notFound(resourceType: "Immunization", id: "(search)")
        case 1:
            let ifMatch = parseETag(request.headers[.ifMatch])
    ...
    // DELETE /Immunization/:id — logical delete
    ...
    group.get { request, _ in
        ...
        let nextURL = result.nextCursor.map { nextImmunizationPageURL(selfURL: base, cursor: $0, count: query.count) }
        let entries = result.entries.map { e -> (fullUrl: String, json: Data) in
            var json = e.jsonWithMeta
            if let s = summary, s != .false { json = applySummary(json, mode: s, summaryFields: immunizationSummaryFields) }
    ...
    // POST /Immunization/_search — form-encoded search
...
func parseImmunizationQuery(from pairs: some Collection<(key: Substring, value: Substring)>) -> ImmunizationSearchQuery {
    ...
    let subject     = first("patient")
    let status      = all("status").flatMap { ImmunizationSearchQuery.TokenParam.parseList(String($0)) }
