...
public struct ServiceRequestStore: Sendable {
    ...
    public struct SearchResult: Sendable {
        ...
        public let nextCursor: ServiceRequestSearchQuery.SearchCursor?
    }
    ...
    public func create(_ sr: ServiceRequest) async throws -> WriteResult {
        let id = UUID().uuidString.lowercased()
        return try await write(id: id, sr: sr, ifMatch: nil)
    }

    public func update(id: String, sr: ServiceRequest, ifMatch: Int64?) async throws -> WriteResult {
        return try await write(id: id, sr: sr, ifMatch: ifMatch)
    }
        ...
        try await client.withConnection { conn in
            ...
            do {
                let rows = try await conn.query(
                    ...
                    WHERE resource_type = 'ServiceRequest' AND id = \(id)
                ...
                guard let current = currentVersion else {
                    ...
                    throw FHIRServerError.notFound(resourceType: "ServiceRequest", id: id)
                }
                if isDeleted {
                    _ = try? await conn.query("ROLLBACK", logger: logger)
                ...
                let insRows = try await conn.query(
                    ...
                    VALUES ('ServiceRequest', \(id), \(nextVersion), now(), '{}'::jsonb, true)
                    RETURNING last_updated
                    """, logger: logger)
                var lastUpdated = Date()
                for try await (d) in insRows.decode(Date.self, context: .default) { lastUpdated = d }

                _ = try await conn.query("DELETE FROM idx_token     WHERE resource_type = 'ServiceRequest' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_string    WHERE resource_type = 'ServiceRequest' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_date      WHERE resource_type = 'ServiceRequest' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_reference WHERE resource_type = 'ServiceRequest' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_quantity  WHERE resource_type = 'ServiceRequest' AND resource_id = \(id)", logger: logger)
    ...
    public func vread(id: String, versionId: Int64) async throws -> ReadResult {
        try await client.withConnection { conn in
            let rows = try await conn.query(
                ...
                FROM resources
                WHERE resource_type = 'ServiceRequest' AND id = \(id) AND version_id = \(versionId)
                """, logger: logger)
            for try await (vid, lastUpdated, content, deleted) in
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                if deleted { throw FHIRServerError.gone(resourceType: "ServiceRequest", id: id) }
    ...
    public func history(id: String) async throws -> [HistoryRawEntry] {
        try await client.withConnection { conn in
                ...
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                ...
                entries.append(HistoryRawEntry(resourceType: "ServiceRequest", id: id, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
        ...
        try await client.withConnection { conn in
            ...
            if let since {
                rows = try await conn.query(
                    ...
                    FROM resources
                    WHERE resource_type = 'ServiceRequest' AND last_updated >= \(since)
            ...
            {
                let jsonData: Data? = deleted ? nil : injectMeta(into: content, versionId: vid, lastUpdated: lastUpdated)
                entries.append(HistoryRawEntry(resourceType: "ServiceRequest", id: rid, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
            return entries
        }
    }
    ...
    public func search(query: ServiceRequestSearchQuery) async throws -> SearchResult {
        if query.count == 0 {
        ...
        return try await client.withConnection { conn in
            ...
            let nextCursor: ServiceRequestSearchQuery.SearchCursor?
            if hasNext, let lastEntry = page.last, let lastSortVal = pageSortVals.last {
                nextCursor = ServiceRequestSearchQuery.SearchCursor(
    ...
    private func write(id: String, sr: ServiceRequest, ifMatch: Int64?) async throws -> WriteResult {
        try validate(sr)
        ...
        return try await client.withConnection { conn in
            ...
            do {
                ...
                let insRows = try await conn.query(
                    ...
                    VALUES ('ServiceRequest', \(id), \(nextVersion), now(), \(jsonString)::jsonb, false)
                    RETURNING last_updated
                    """, logger: logger)
                var lastUpdated = Date()
                for try await (d) in insRows.decode(Date.self, context: .default) { lastUpdated = d }
    ...
    private func validate(_ sr: ServiceRequest) throws {}
    ...
    private func replaceIndexRows(conn: PostgresConnection, id: String, params: SearchParams) async throws {
        ...
        for row in params.tokens {
            _ = try await conn.query(
                "INSERT INTO idx_token (resource_type, resource_id, param_name, system, code) VALUES ('ServiceRequest', \(id), \(row.paramName), \(row.system), \(row.code))",
                logger: logger)
        }
        for row in params.strings {
            _ = try await conn.query(
                "INSERT INTO idx_string (resource_type, resource_id, param_name, value) VALUES ('ServiceRequest', \(id), \(row.paramName), \(row.value))",
                logger: logger)
        }
        for row in params.dates {
            _ = try await conn.query(
                "INSERT INTO idx_date (resource_type, resource_id, param_name, date_start, date_end) VALUES ('ServiceRequest', \(id), \(row.paramName), \(row.dateStart), \(row.dateEnd))",
                logger: logger)
        }
        for row in params.references {
            _ = try await conn.query(
                "INSERT INTO idx_reference (resource_type, resource_id, param_name, ref_type, ref_id) VALUES ('ServiceRequest', \(id), \(row.paramName), \(row.refType), \(row.refId))",
                logger: logger)
        }
        for row in params.quantities {
            _ = try await conn.query(
                "INSERT INTO idx_quantity (resource_type, resource_id, param_name, system, code, value) VALUES ('ServiceRequest', \(id), \(row.paramName), \(row.system), \(row.code), \(row.value))",
                logger: logger)
        }
    }

    private func buildSearchSQL(query: ServiceRequestSearchQuery) throws -> (String, PostgresBindings) {
        ...
        func tokenORCTE(name: String, paramName: String, tokens: [ServiceRequestSearchQuery.TokenParam]) -> (String, String) {
        ...
        func tokenNotCondition(paramName: String, tokens: [ServiceRequestSearchQuery.TokenParam]) -> String {
            var orClauses: [String] = []
            for tok in tokens {
                if tok.code.isEmpty, let sys = tok.system {
                    orClauses.append("system = \(bind(sys))")
        ...
        func refCTE(name: String, paramName: String, ref: String) -> (String, String) {
            ...
            if parts.count == 2 {
                ...
                return (name, "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'ServiceRequest' AND param_name = '\(paramName)' AND ref_type = \(refTypeP) AND ref_id = \(refIdP)")
            } else {
                let refIdP = bind(ref)
                return (name, "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'ServiceRequest' AND param_name = '\(paramName)' AND ref_id = \(refIdP)")
            }
        }

        func dateCTE(name: String, paramName: String, dp: ServiceRequestSearchQuery.DateParam) -> (String, String) {
            ...
            return (name, "SELECT DISTINCT resource_id FROM idx_date WHERE resource_type = 'ServiceRequest' AND param_name = '\(paramName)' AND \(cond)")
        }
        ...
        if !query.bodySite.isEmpty      { filterCTEs.append(tokenORCTE(name: "f_bodysite",  paramName: "body-site",      tokens: query.bodySite)) }
        if !query.performerType.isEmpty { filterCTEs.append(tokenORCTE(name: "f_perftype",  paramName: "performer-type", tokens: query.performerType)) }
        if !query.requisition.isEmpty   { filterCTEs.append(tokenORCTE(name: "f_req",       paramName: "requisition",    tokens: query.requisition)) }
        ...
        if !query.identifier.isEmpty {
            ...
            if !orClauses.isEmpty {
                filterCTEs.append(("f_ident", "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR ")))"))
            }
        }
        ...
        for (i, dp) in query.occurrence.enumerated() {
            filterCTEs.append(dateCTE(name: "f_occ\(i)", paramName: "occurrence", dp: dp))
        }
        ...
        if let ref = query.requester { filterCTEs.append(refCTE(name: "f_requester", paramName: "requester", ref: ref)) }
        ...
        if let ref = query.specimen  { filterCTEs.append(refCTE(name: "f_specimen",  paramName: "specimen",  ref: ref)) }
        ...
        case .dateAscending, .dateDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_date WHERE resource_type = 'ServiceRequest' AND param_name = 'authored' " +
        ...
        case .codeAscending, .codeDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'code' " +
        ...
        case .statusAscending, .statusDescending:
            sortKeysCTE = ("sort_keys",
                "SELECT DISTINCT ON (resource_id) resource_id, code AS sv " +
                "FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'status' " +
                "ORDER BY resource_id, code ASC")
            if let cursor = query.cursor {
                let codeP = bind(cursor.sortValue)
                let idP = bind(cursor.id)
        ...
        let fromClause = skipTotal
            ? "FROM paged p JOIN resources r ON r.resource_type = 'ServiceRequest' AND r.id = p.id AND r.version_id = p.version_id"
            : "FROM paged p CROSS JOIN total_count t JOIN resources r ON r.resource_type = 'ServiceRequest' AND r.id = p.id AND r.version_id = p.version_id"

        let sql = "\(withClause)\nSELECT p.id, p.version_id, p.last_updated, r.content, \(totalExpr), \(finalSortValSQL) AS sort_val_text\n\(fromClause)"
        return (sql, binds)
    }

    private func buildCountSQL(query: ServiceRequestSearchQuery) throws -> (String, PostgresBindings) {
        var binds = PostgresBindings()
        var n = 0
        func bind(_ val: some PostgresDynamicTypeEncodable) -> String {
            n += 1; binds.append(val); return "$\(n)"
        ...
        func countTokenORCTE(name: String, paramName: String, tokens: [ServiceRequestSearchQuery.TokenParam]) -> (String, String) {
            var orClauses: [String] = []
            for tok in tokens {
                if tok.code.isEmpty, let sys = tok.system {
                    orClauses.append("system = \(bind(sys))")
        ...
        func countDateCTE(name: String, paramName: String, dp: ServiceRequestSearchQuery.DateParam) -> (String, String) {
            let startP = bind(dp.dateStart); let endP = bind(dp.dateEnd)
            let cond: String
            switch dp.prefix {
            case .eq: cond = "date_start >= \(startP) AND date_end <= \(endP)"
        ...
        if !query.intent.isEmpty        { filterCTEs.append(countTokenORCTE(name: "f_intent",    paramName: "intent",         tokens: query.intent)) }
        if !query.priority.isEmpty      { filterCTEs.append(countTokenORCTE(name: "f_priority",  paramName: "priority",       tokens: query.priority)) }
        if !query.code.isEmpty          { filterCTEs.append(countTokenORCTE(name: "f_code",      paramName: "code",           tokens: query.code)) }
        if !query.category.isEmpty      { filterCTEs.append(countTokenORCTE(name: "f_category",  paramName: "category",       tokens: query.category)) }
        if !query.bodySite.isEmpty      { filterCTEs.append(countTokenORCTE(name: "f_bodysite",  paramName: "body-site",      tokens: query.bodySite)) }
        if !query.performerType.isEmpty { filterCTEs.append(countTokenORCTE(name: "f_perftype",  paramName: "performer-type", tokens: query.performerType)) }
        if !query.requisition.isEmpty   { filterCTEs.append(countTokenORCTE(name: "f_req",       paramName: "requisition",    tokens: query.requisition)) }
        ...
        for (i, dp) in query.occurrence.enumerated() {
            filterCTEs.append(countDateCTE(name: "f_occ\(i)", paramName: "occurrence", dp: dp))
        }
        ...
        if let ref = query.requester { filterCTEs.append(countRefCTE(name: "f_requester", paramName: "requester", ref: ref)) }
        if let ref = query.performer { filterCTEs.append(countRefCTE(name: "f_performer", paramName: "performer", ref: ref)) }
        if let ref = query.basedOn   { filterCTEs.append(countRefCTE(name: "f_basedon",   paramName: "based-on",  ref: ref)) }
    ...
    private func serviceRequestMissingSubquery(param: String) -> String? {
        switch param {
        case "status":         return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'status'"
        case "intent":         return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'intent'"
        case "priority":       return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'priority'"
        case "code":           return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'code'"
        case "category":       return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'category'"
        case "body-site":      return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'body-site'"
        case "performer-type": return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'performer-type'"
        case "requisition":    return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'requisition'"
        case "identifier":     return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'ServiceRequest' AND param_name = 'identifier'"
        case "authored":       return "SELECT DISTINCT resource_id FROM idx_date WHERE resource_type = 'ServiceRequest' AND param_name = 'authored'"
        case "occurrence":     return "SELECT DISTINCT resource_id FROM idx_date WHERE resource_type = 'ServiceRequest' AND param_name = 'occurrence'"
        case "subject":        return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'ServiceRequest' AND param_name = 'subject'"
        case "patient":        return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'ServiceRequest' AND param_name = 'patient'"
        case "encounter":      return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'ServiceRequest' AND param_name = 'encounter'"
        case "requester":      return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'ServiceRequest' AND param_name = 'requester'"
