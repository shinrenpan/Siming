    ...
    public func delete(id: String, ifMatch: Int64?) async throws -> DeleteResult {
        try await client.withConnection { conn in
            let (versionId, lastUpdated, _) = try await deleteResource(
                conn: conn, resourceType: "ServiceRequest", id: id, ifMatch: ifMatch, logger: logger)
        ...
        try await client.withConnection { conn in
            ...
            if let since {
                rows = try await conn.query(
                    ...
                    SELECT version_id, last_updated, content, deleted
                    FROM resources
                    WHERE resource_type = 'ServiceRequest' AND id = \(id) AND last_updated >= \(since)
            ...
            if entries.isEmpty {
                if since != nil {
                    let existRows = try await conn.query(
                        """
                        SELECT 1 FROM resources WHERE resource_type = 'ServiceRequest' AND id = \(id) LIMIT 1
                        """, logger: logger)
                    var exists = false
                    for try await _ in existRows { exists = true }
                    if !exists { throw FHIRServerError.notFound(resourceType: "ServiceRequest", id: id) }
                } else {
                    throw FHIRServerError.notFound(resourceType: "ServiceRequest", id: id)
                }
            }
    ...
    private func write(id: String, sr: ServiceRequest, ifMatch: Int64?) async throws -> WriteResult {
        ...
        return try await client.withConnection { conn in
            let (versionId, lastUpdated) = try await writeResource(
                conn: conn, resourceType: "ServiceRequest", id: id,
    ...
    private func buildSearchSQL(query: ServiceRequestSearchQuery) throws -> (String, PostgresBindings) {
        ...
        for (i, tt) in query.tokenTexts.enumerated() {
            ...
            filterCTEs.append(("f_ttext\(i)",
                "SELECT DISTINCT resource_id FROM idx_string WHERE resource_type = 'ServiceRequest' AND param_name = \(pn) AND value ILIKE \(val)"))
        }
