...
public struct MedicationAdministrationStore: Sendable {
    ...
    public struct SearchResult: Sendable {
        ...
        public let nextCursor: MedicationAdministrationSearchQuery.SearchCursor?
    }
    ...
    public func create(_ ma: MedicationAdministration) async throws -> WriteResult {
        let id = UUID().uuidString.lowercased()
        return try await write(id: id, medicationAdministration: ma, ifMatch: nil)
    }

    public func update(id: String, medicationAdministration: MedicationAdministration, ifMatch: Int64?) async throws -> WriteResult {
        return try await write(id: id, medicationAdministration: medicationAdministration, ifMatch: ifMatch)
    }
        ...
        try await client.withConnection { conn in
            ...
            do {
                let rows = try await conn.query(
                    ...
                    WHERE resource_type = 'MedicationAdministration' AND id = \(id)
                ...
                guard let current = currentVersion else {
                    ...
                    throw FHIRServerError.notFound(resourceType: "MedicationAdministration", id: id)
                }
                if isDeleted {
                    _ = try? await conn.query("ROLLBACK", logger: logger)
                ...
                let insRows = try await conn.query(
                    ...
                    VALUES ('MedicationAdministration', \(id), \(nextVersion), now(), '{}'::jsonb, true)
                    RETURNING last_updated
                    """, logger: logger)
                var lastUpdated = Date()
                for try await (d) in insRows.decode(Date.self, context: .default) { lastUpdated = d }

                _ = try await conn.query("DELETE FROM idx_token     WHERE resource_type = 'MedicationAdministration' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_string    WHERE resource_type = 'MedicationAdministration' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_date      WHERE resource_type = 'MedicationAdministration' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_reference WHERE resource_type = 'MedicationAdministration' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_quantity  WHERE resource_type = 'MedicationAdministration' AND resource_id = \(id)", logger: logger)
    ...
    public func vread(id: String, versionId: Int64) async throws -> ReadResult {
        try await client.withConnection { conn in
            let rows = try await conn.query(
                ...
                FROM resources
                WHERE resource_type = 'MedicationAdministration' AND id = \(id) AND version_id = \(versionId)
                """, logger: logger)
            for try await (vid, lastUpdated, content, deleted) in
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                if deleted { throw FHIRServerError.gone(resourceType: "MedicationAdministration", id: id) }
    ...
    public func history(id: String) async throws -> [HistoryRawEntry] {
        try await client.withConnection { conn in
                ...
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                ...
                entries.append(HistoryRawEntry(resourceType: "MedicationAdministration", id: id, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
        ...
        try await client.withConnection { conn in
            ...
            if let since {
                rows = try await conn.query(
                    ...
                    FROM resources
                    WHERE resource_type = 'MedicationAdministration' AND last_updated >= \(since)
            ...
            {
                let jsonData: Data? = deleted ? nil : injectMeta(into: content, versionId: vid, lastUpdated: lastUpdated)
                entries.append(HistoryRawEntry(resourceType: "MedicationAdministration", id: rid, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
            return entries
        }
    }
    ...
    public func search(query: MedicationAdministrationSearchQuery) async throws -> SearchResult {
        if query.count == 0 {
        ...
        return try await client.withConnection { conn in
            ...
            let nextCursor: MedicationAdministrationSearchQuery.SearchCursor?
            if hasNext, let lastEntry = page.last, let lastSortVal = pageSortVals.last {
                nextCursor = MedicationAdministrationSearchQuery.SearchCursor(
    ...
    private func write(id: String, medicationAdministration: MedicationAdministration, ifMatch: Int64?) async throws -> WriteResult {
        try validate(medicationAdministration)

        var ma = medicationAdministration
        ma.id   = FHIRPrimitive(FHIRString(id))
        ma.meta = nil

        let jsonData   = try JSONEncoder().encode(ma)
        let jsonString = String(data: jsonData, encoding: .utf8)!
        let searchParams = extractMedicationAdministrationSearchParams(ma)
        ...
        return try await client.withConnection { conn in
            ...
            do {
                ...
                let insRows = try await conn.query(
                    ...
                    VALUES ('MedicationAdministration', \(id), \(nextVersion), now(), \(jsonString)::jsonb, false)
                    RETURNING last_updated
                    """, logger: logger)
                var lastUpdated = Date()
                for try await (d) in insRows.decode(Date.self, context: .default) { lastUpdated = d }
    ...
    private func validate(_ ma: MedicationAdministration) throws {}
    ...
    private func replaceIndexRows(conn: PostgresConnection, id: String, params: SearchParams) async throws {
        ...
        for row in params.tokens {
            _ = try await conn.query(
                "INSERT INTO idx_token (resource_type, resource_id, param_name, system, code) VALUES ('MedicationAdministration', \(id), \(row.paramName), \(row.system), \(row.code))",
                logger: logger)
        }
        for row in params.strings {
            _ = try await conn.query(
                "INSERT INTO idx_string (resource_type, resource_id, param_name, value) VALUES ('MedicationAdministration', \(id), \(row.paramName), \(row.value))",
                logger: logger)
        }
        for row in params.dates {
            _ = try await conn.query(
                "INSERT INTO idx_date (resource_type, resource_id, param_name, date_start, date_end) VALUES ('MedicationAdministration', \(id), \(row.paramName), \(row.dateStart), \(row.dateEnd))",
                logger: logger)
        }
        for row in params.references {
            _ = try await conn.query(
                "INSERT INTO idx_reference (resource_type, resource_id, param_name, ref_type, ref_id) VALUES ('MedicationAdministration', \(id), \(row.paramName), \(row.refType), \(row.refId))",
                logger: logger)
        }
        for row in params.quantities {
            _ = try await conn.query(
                "INSERT INTO idx_quantity (resource_type, resource_id, param_name, system, code, value) VALUES ('MedicationAdministration', \(id), \(row.paramName), \(row.system), \(row.code), \(row.value))",
                logger: logger)
        }
    }

    private func buildSearchSQL(query: MedicationAdministrationSearchQuery) throws -> (String, PostgresBindings) {
        ...
        func tokenORCTE(name: String, paramName: String, tokens: [MedicationAdministrationSearchQuery.TokenParam]) -> (String, String) {
            ...
            return (name, "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'MedicationAdministration' AND param_name = '\(paramName)' AND (\(orClauses.joined(separator: " OR ")))")
        }

        func tokenNotCondition(paramName: String, tokens: [MedicationAdministrationSearchQuery.TokenParam]) -> String {
            var orClauses: [String] = []
            for tok in tokens {
                if tok.code.isEmpty, let sys = tok.system {
                    orClauses.append("system = \(bind(sys))")
            ...
            return "r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'MedicationAdministration' AND param_name = '\(paramName)' AND (\(orClauses.joined(separator: " OR "))))"
        }
        ...
        func refCTE(name: String, paramName: String, ref: String) -> (String, String) {
            ...
            if parts.count == 2 {
                ...
                return (name, "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'MedicationAdministration' AND param_name = '\(paramName)' AND ref_type = \(refTypeP) AND ref_id = \(refIdP)")
            } else {
                let refIdP = bind(ref)
                return (name, "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'MedicationAdministration' AND param_name = '\(paramName)' AND ref_id = \(refIdP)")
            }
        }

        func dateCTE(name: String, paramName: String, dp: MedicationAdministrationSearchQuery.DateParam) -> (String, String) {
            ...
            return (name, "SELECT DISTINCT resource_id FROM idx_date WHERE resource_type = 'MedicationAdministration' AND param_name = '\(paramName)' AND \(cond)")
        }
        ...
        if !query.reasonGiven.isEmpty    { filterCTEs.append(tokenORCTE(name: "f_rg",        paramName: "reason-given",   tokens: query.reasonGiven)) }
        if !query.reasonNotGiven.isEmpty { filterCTEs.append(tokenORCTE(name: "f_rng",       paramName: "reason-not-given", tokens: query.reasonNotGiven)) }
        ...
        if !query.identifier.isEmpty {
            ...
            if !orClauses.isEmpty {
                filterCTEs.append(("f_ident", "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'MedicationAdministration' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR ")))"))
            }
        }

        // date CTEs
        for (i, dp) in query.effectiveTime.enumerated() {
            filterCTEs.append(dateCTE(name: "f_eff\(i)", paramName: "effective-time", dp: dp))
        }
        ...
        if let ref = query.request   { filterCTEs.append(refCTE(name: "f_request",   paramName: "request",    ref: ref)) }
        if let ref = query.performer { filterCTEs.append(refCTE(name: "f_performer", paramName: "performer",  ref: ref)) }
        if let ref = query.device    { filterCTEs.append(refCTE(name: "f_device",    paramName: "device",     ref: ref)) }
        ...
        for (i, chain) in query.chains.enumerated() {
            if let (name, sql) = chainFilterCTE(
                index: filterCTEs.count + i, sourceType: "MedicationAdministration",
        ...
        for (i, hp) in query.has.enumerated() {
            if let (name, sql) = hasFilterCTE(
                index: i, mainType: "MedicationAdministration",
        ...
        case .dateAscending, .dateDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_date WHERE resource_type = 'MedicationAdministration' AND param_name = 'effective-time' " +
        ...
        case .statusAscending, .statusDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_token WHERE resource_type = 'MedicationAdministration' AND param_name = 'status' " +
        ...
        case .codeAscending, .codeDescending:
            sortKeysCTE = ("sort_keys",
                "SELECT DISTINCT ON (resource_id) resource_id, code AS sv " +
                "FROM idx_token WHERE resource_type = 'MedicationAdministration' AND param_name = 'code' " +
                "ORDER BY resource_id, code ASC")
            if let cursor = query.cursor {
                let codeP = bind(cursor.sortValue)
                let idP = bind(cursor.id)
        ...
        let fromClause = skipTotal
            ? "FROM paged p JOIN resources r ON r.resource_type = 'MedicationAdministration' AND r.id = p.id AND r.version_id = p.version_id"
            : "FROM paged p CROSS JOIN total_count t JOIN resources r ON r.resource_type = 'MedicationAdministration' AND r.id = p.id AND r.version_id = p.version_id"

        let sql = "\(withClause)\nSELECT p.id, p.version_id, p.last_updated, r.content, \(totalExpr), \(finalSortValSQL) AS sort_val_text\n\(fromClause)"
        return (sql, binds)
    }

    private func buildCountSQL(query: MedicationAdministrationSearchQuery) throws -> (String, PostgresBindings) {
        var binds = PostgresBindings()
        var n = 0
        func bind(_ val: some PostgresDynamicTypeEncodable) -> String {
            n += 1; binds.append(val); return "$\(n)"
        ...
        func tokenCTE(name: String, paramName: String, tokens: [MedicationAdministrationSearchQuery.TokenParam]) -> (String, String) {
            var orClauses: [String] = []
            for tok in tokens {
                if tok.code.isEmpty, let sys = tok.system {
                    orClauses.append("system = \(bind(sys))")
        ...
        func cDateCTE(name: String, paramName: String, dp: MedicationAdministrationSearchQuery.DateParam) -> (String, String) {
            let startP = bind(dp.dateStart); let endP = bind(dp.dateEnd)
            let cond: String
            switch dp.prefix {
            case .eq: cond = "date_start <= \(endP) AND date_end >= \(startP)"
        ...
        if !query.reasonGiven.isEmpty    { filterCTEs.append(tokenCTE(name: "f_rg",      paramName: "reason-given",     tokens: query.reasonGiven)) }
        if !query.reasonNotGiven.isEmpty { filterCTEs.append(tokenCTE(name: "f_rng",     paramName: "reason-not-given", tokens: query.reasonNotGiven)) }
        ...
        for (i, dp) in query.effectiveTime.enumerated() {
            filterCTEs.append(cDateCTE(name: "f_eff\(i)", paramName: "effective-time", dp: dp))
        }
        ...
        if let ref = query.request    { filterCTEs.append(cRefCTE(name: "f_request",    paramName: "request",    ref: ref)) }
        if let ref = query.performer  { filterCTEs.append(cRefCTE(name: "f_performer",  paramName: "performer",  ref: ref)) }
        if let ref = query.device     { filterCTEs.append(cRefCTE(name: "f_device",     paramName: "device",     ref: ref)) }
    ...
    private func medicationAdministrationMissingSubquery(param: String) -> String? {
        switch param {
        case "subject", "patient": return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'MedicationAdministration' AND param_name IN ('subject', 'patient')"
        case "status":             return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'MedicationAdministration' AND param_name = 'status'"
        case "code":               return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'MedicationAdministration' AND param_name = 'code'"
        case "reason-given":       return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'MedicationAdministration' AND param_name = 'reason-given'"
        case "reason-not-given":   return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'MedicationAdministration' AND param_name = 'reason-not-given'"
        case "identifier":         return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'MedicationAdministration' AND param_name = 'identifier'"
        case "effective-time":     return "SELECT DISTINCT resource_id FROM idx_date WHERE resource_type = 'MedicationAdministration' AND param_name = 'effective-time'"
        case "context":            return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'MedicationAdministration' AND param_name = 'context'"
        case "request":            return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'MedicationAdministration' AND param_name = 'request'"
        case "performer":          return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'MedicationAdministration' AND param_name = 'performer'"
        case "device":             return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'MedicationAdministration' AND param_name = 'device'"
        case "medication":         return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'MedicationAdministration' AND param_name = 'medication'"
        default:                   return nil
        }
    }
}
