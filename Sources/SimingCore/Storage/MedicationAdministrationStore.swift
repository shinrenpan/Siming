...
public struct MedicationAdministrationStore: Sendable {
    ...
    private func write(id: String, medicationAdministration: MedicationAdministration, ifMatch: Int64?) async throws -> WriteResult {
        ...
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "MedicationAdministration", json: _jsonObj, terminology: terminology)
        }
    ...
    private func buildCountSQL(query: MedicationAdministrationSearchQuery) throws -> (String, PostgresBindings) {
        ...
        let idsInner = buildCountIdsInner(
            resourceType: "MedicationAdministration", filterCTEs: filterCTEs, whereConditions: whereConditions)
