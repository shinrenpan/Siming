...
public struct ImmunizationStore: Sendable {
    ...
    private func write(id: String, immunization: Immunization, ifMatch: Int64?) async throws -> WriteResult {
        ...
        let originalMeta = imm.meta
        imm.meta = nil

        let jsonData   = try JSONEncoder().encode(imm)
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "Immunization", json: _jsonObj, terminology: terminology)
        }
        let jsonString = String(data: jsonData, encoding: .utf8)!
        var searchParams = extractImmunizationSearchParams(imm)
