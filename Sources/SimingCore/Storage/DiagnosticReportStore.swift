...
public struct DiagnosticReportStore: Sendable {
    ...
    private func write(id: String, diagnosticReport: DiagnosticReport, ifMatch: Int64?) async throws -> WriteResult {
        ...
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "DiagnosticReport", json: _jsonObj, terminology: terminology)
        }
    ...
    private func buildSearchSQL(query: DiagnosticReportSearchQuery) throws -> (String, PostgresBindings) {
        ...
        if !query.identifierNot.isEmpty {
            ...
            if !orClauses.isEmpty {
                extraConditions.append("r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR "))))")
            }
        }
        ...
        if !query.conclusionNot.isEmpty { extraConditions.append(notTokenCond(paramName: "conclusion", tokens: query.conclusionNot)) }
    ...
    private func buildCountSQL(query: DiagnosticReportSearchQuery) throws -> (String, PostgresBindings) {
        ...
        let idsInner = buildCountIdsInner(
            resourceType: "DiagnosticReport", filterCTEs: filterCTEs, whereConditions: whereConditions)
