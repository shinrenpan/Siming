...
public struct DiagnosticReportStore: Sendable {
    ...
    public struct SearchResult: Sendable {
        ...
        public let nextCursor: DiagnosticReportSearchQuery.SearchCursor?
    }
    ...
    public func create(_ dr: DiagnosticReport) async throws -> WriteResult {
        let id = UUID().uuidString.lowercased()
        return try await write(id: id, diagnosticReport: dr, ifMatch: nil)
    }

    public func update(id: String, diagnosticReport: DiagnosticReport, ifMatch: Int64?) async throws -> WriteResult {
        return try await write(id: id, diagnosticReport: diagnosticReport, ifMatch: ifMatch)
    }
    ...
    public func delete(id: String, ifMatch: Int64?) async throws -> DeleteResult {
        try await client.withConnection { conn in
            let (versionId, lastUpdated, _) = try await deleteResource(
                conn: conn, resourceType: "DiagnosticReport", id: id, ifMatch: ifMatch, logger: logger)
    ...
    public func vread(id: String, versionId: Int64) async throws -> ReadResult {
        try await client.withConnection { conn in
            let rows = try await conn.query(
                ...
                FROM resources
                WHERE resource_type = 'DiagnosticReport' AND id = \(id) AND version_id = \(versionId)
                """, logger: logger)
            for try await (vid, lastUpdated, content, deleted) in
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                if deleted { throw FHIRServerError.gone(resourceType: "DiagnosticReport", id: id) }
            ...
            throw FHIRServerError.notFound(resourceType: "DiagnosticReport", id: id)
        }
    }
        ...
        try await client.withConnection { conn in
            ...
            if let since {
                rows = try await conn.query(
                    ...
                    SELECT version_id, last_updated, content, deleted
                    FROM resources
                    WHERE resource_type = 'DiagnosticReport' AND id = \(id) AND last_updated >= \(since)
                ...
                rows = try await conn.query(
                    ...
                    SELECT version_id, last_updated, content, deleted
                    FROM resources
                    WHERE resource_type = 'DiagnosticReport' AND id = \(id)
                    ORDER BY version_id DESC
                    LIMIT \(Int64(count))
                    """, logger: logger)
            }
                ...
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                ...
                entries.append(HistoryRawEntry(resourceType: "DiagnosticReport", id: id, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
            if entries.isEmpty {
                if since != nil {
                    let existRows = try await conn.query(
                        """
                        SELECT 1 FROM resources WHERE resource_type = 'DiagnosticReport' AND id = \(id) LIMIT 1
                        """, logger: logger)
                    var exists = false
                    for try await _ in existRows { exists = true }
                    if !exists { throw FHIRServerError.notFound(resourceType: "DiagnosticReport", id: id) }
                } else {
                    throw FHIRServerError.notFound(resourceType: "DiagnosticReport", id: id)
                }
            }
        ...
        try await client.withConnection { conn in
            ...
            if let since {
                rows = try await conn.query(
                    ...
                    FROM resources
                    WHERE resource_type = 'DiagnosticReport' AND last_updated >= \(since)
            ...
            {
                let jsonData: Data? = deleted ? nil : injectMeta(into: content, versionId: vid, lastUpdated: lastUpdated)
                entries.append(HistoryRawEntry(resourceType: "DiagnosticReport", id: rid, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
            return entries
        }
    }
    ...
    public func search(query: DiagnosticReportSearchQuery) async throws -> SearchResult {
        if query.count == 0 {
        ...
        return try await client.withConnection { conn in
            ...
            let nextCursor: DiagnosticReportSearchQuery.SearchCursor?
            if hasNext, let lastEntry = page.last, let lastSortVal = pageSortVals.last {
                nextCursor = DiagnosticReportSearchQuery.SearchCursor(
    ...
    private func write(id: String, diagnosticReport: DiagnosticReport, ifMatch: Int64?) async throws -> WriteResult {
        try validate(diagnosticReport)
        ...
        return try await client.withConnection { conn in
            let (versionId, lastUpdated) = try await writeResource(
                conn: conn, resourceType: "DiagnosticReport", id: id,
    ...
    private func validate(_ dr: DiagnosticReport) throws {}

    private func buildSearchSQL(query: DiagnosticReportSearchQuery) throws -> (String, PostgresBindings) {
        ...
        if let subject = query.subject {
            ...
            if parts.count == 2 {
                ...
                filterCTEs.append(("f_subject", """
                    ...
                    WHERE resource_type = 'DiagnosticReport' AND param_name IN ('patient', 'subject')
        ...
        if let enc = query.encounter {
            ...
            if parts.count == 2 {
                ...
                filterCTEs.append(("f_encounter", """
                    SELECT DISTINCT resource_id FROM idx_reference
                    WHERE resource_type = 'DiagnosticReport' AND param_name = 'encounter'
                      AND ref_type = \(refTypeP) AND ref_id = \(refIdP)
                    """))
            } else {
                let refIdP = bind(enc)
        ...
        if let perf = query.performer {
            ...
            if parts.count == 2 {
                ...
                filterCTEs.append(("f_performer", """
                    SELECT DISTINCT resource_id FROM idx_reference
                    WHERE resource_type = 'DiagnosticReport' AND param_name = 'performer'
                      AND ref_type = \(refTypeP) AND ref_id = \(refIdP)
                    """))
            } else {
        ...
        func refCTE(name: String, paramName: String, ref: String) -> (String, String) {
            ...
            if parts.count == 2 {
                let refTypeP = bind(String(parts[0])); let refIdP = bind(String(parts[1]))
                return (name, "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = '\(paramName)' AND ref_type = \(refTypeP) AND ref_id = \(refIdP)")
            } else {
                let refIdP = bind(ref)
                return (name, "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = '\(paramName)' AND ref_id = \(refIdP)")
            }
        }

        if let basedOn = query.basedOn { filterCTEs.append(refCTE(name: "f_based_on", paramName: "based-on", ref: basedOn)) }
        if let media = query.media { filterCTEs.append(refCTE(name: "f_media", paramName: "media", ref: media)) }
        if let result = query.result { filterCTEs.append(refCTE(name: "f_result", paramName: "result", ref: result)) }
        if let ri = query.resultsInterpreter { filterCTEs.append(refCTE(name: "f_results_interp", paramName: "results-interpreter", ref: ri)) }
        if let specimen = query.specimen { filterCTEs.append(refCTE(name: "f_specimen", paramName: "specimen", ref: specimen)) }

        func tokenORCTE(name: String, paramName: String, tokens: [DiagnosticReportSearchQuery.TokenParam]) -> (String, String) {
            ...
            return (name,
                "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = '\(paramName)' AND (\(orClauses.joined(separator: " OR ")))")
        }
        ...
        if !query.conclusion.isEmpty { filterCTEs.append(tokenORCTE(name: "f_conclusion", paramName: "conclusion", tokens: query.conclusion)) }
        ...
        if !query.identifier.isEmpty {
            ...
            if !orClauses.isEmpty {
                filterCTEs.append(("f_ident",
                    "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR ")))"))
            }
        }

        func dateCTE(prefix: String, paramName: String, dp: DiagnosticReportSearchQuery.DateParam, idx: Int) -> (String, String) {
        ...
        for (i, dp) in query.issued.enumerated() { filterCTEs.append(dateCTE(prefix: "f_issued", paramName: "issued", dp: dp, idx: i)) }

        // ── WHERE conditions ──────────────────────────────────────────────────

        var whereConditions = ["r.resource_type = 'DiagnosticReport'", "r.deleted = false"]
        ...
        func notTokenCond(paramName: String, tokens: [DiagnosticReportSearchQuery.TokenParam]) -> String {
            var orClauses: [String] = []
            for tok in tokens {
                if tok.code.isEmpty, let sys = tok.system {
                    orClauses.append("system = \(bind(sys))")
            ...
            return "r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = '\(paramName)' AND (\(orClauses.joined(separator: " OR "))))"
        }
        ...
        if !query.identifierNot.isEmpty {
            ...
            if !orClauses.isEmpty {
                whereConditions.append("r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR "))))")
            }
        }
        ...
        if !query.conclusionNot.isEmpty { whereConditions.append(notTokenCond(paramName: "conclusion", tokens: query.conclusionNot)) }

        for paramName in query.missing.keys.sorted() {
            if let sub = diagnosticReportMissingSubquery(param: paramName) {
        ...
        for (i, chain) in query.chains.enumerated() {
            if let (name, sql) = chainFilterCTE(
                index: filterCTEs.count + i, sourceType: "DiagnosticReport",
        ...
        for (i, hp) in query.has.enumerated() {
            if let (name, sql) = hasFilterCTE(
                index: i, mainType: "DiagnosticReport",
        ...
        for (i, tt) in query.tokenTexts.enumerated() {
            ...
            filterCTEs.append(("f_ttext\(i)",
                "SELECT DISTINCT resource_id FROM idx_string WHERE resource_type = 'DiagnosticReport' AND param_name = \(pn) AND value ILIKE \(val)"))
        }
        ...
        case .dateAscending, .dateDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_date WHERE resource_type = 'DiagnosticReport' AND param_name = 'date' " +
        ...
        case .statusAscending, .statusDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = 'status' " +
        ...
        case .codeAscending, .codeDescending:
            sortKeysCTE = ("sort_keys",
                "SELECT DISTINCT ON (resource_id) resource_id, code AS sv " +
                "FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = 'code' " +
                "ORDER BY resource_id, code ASC")
            if let cursor = query.cursor {
    ...
    private func buildCountSQL(query: DiagnosticReportSearchQuery) throws -> (String, PostgresBindings) {
        var binds = PostgresBindings()
        var n = 0
        func bind(_ val: some PostgresDynamicTypeEncodable) -> String {
            n += 1; binds.append(val); return "$\(n)"
        ...
        if let subject = query.subject {
            ...
            if parts.count == 2 {
                let refTypeP = bind(String(parts[0])); let refIdP = bind(String(parts[1]))
                filterCTEs.append(("f_subject",
                    "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name IN ('patient', 'subject') AND ref_type = \(refTypeP) AND ref_id = \(refIdP)"))
            } else {
                let refIdP = bind(subject)
                filterCTEs.append(("f_subject",
                    "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name IN ('patient', 'subject') AND ref_id = \(refIdP)"))
            }
        }
        ...
        if let enc = query.encounter {
            ...
            if parts.count == 2 {
                let refTypeP = bind(String(parts[0])); let refIdP = bind(String(parts[1]))
                filterCTEs.append(("f_encounter",
                    "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'encounter' AND ref_type = \(refTypeP) AND ref_id = \(refIdP)"))
            } else {
                let refIdP = bind(enc)
                filterCTEs.append(("f_encounter",
                    "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'encounter' AND ref_id = \(refIdP)"))
            }
        }
        ...
        if let perf = query.performer {
            ...
            if parts.count == 2 {
                let refTypeP = bind(String(parts[0])); let refIdP = bind(String(parts[1]))
                filterCTEs.append(("f_performer",
                    "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'performer' AND ref_type = \(refTypeP) AND ref_id = \(refIdP)"))
            } else {
                let refIdP = bind(perf)
                filterCTEs.append(("f_performer",
                    "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'performer' AND ref_id = \(refIdP)"))
            }
        }
        ...
        if let media = query.media { filterCTEs.append(refCTECount(name: "f_media", paramName: "media", ref: media)) }
        if let result = query.result { filterCTEs.append(refCTECount(name: "f_result", paramName: "result", ref: result)) }
        if let ri = query.resultsInterpreter { filterCTEs.append(refCTECount(name: "f_results_interp", paramName: "results-interpreter", ref: ri)) }
        if let specimen = query.specimen { filterCTEs.append(refCTECount(name: "f_specimen", paramName: "specimen", ref: specimen)) }

        func tokenCTE(name: String, paramName: String, tokens: [DiagnosticReportSearchQuery.TokenParam]) -> (String, String) {
            var orClauses: [String] = []
            for tok in tokens {
                if tok.code.isEmpty, let sys = tok.system {
                    orClauses.append("system = \(bind(sys))")
            ...
            return (name, "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = '\(paramName)' AND (\(orClauses.joined(separator: " OR ")))")
        }
        ...
        if !query.conclusion.isEmpty { filterCTEs.append(tokenCTE(name: "f_conclusion", paramName: "conclusion", tokens: query.conclusion)) }

        func dateCTECount(prefix: String, paramName: String, dp: DiagnosticReportSearchQuery.DateParam, idx: Int) -> (String, String) {
            let startP = bind(dp.dateStart); let endP = bind(dp.dateEnd)
            let cond: String
            switch dp.prefix {
            case .eq: cond = "date_start <= \(endP) AND date_end >= \(startP)"
        ...
        for (i, dp) in query.issued.enumerated() { filterCTEs.append(dateCTECount(prefix: "f_issued", paramName: "issued", dp: dp, idx: i)) }
    ...
    private func diagnosticReportMissingSubquery(param: String) -> String? {
        switch param {
        case "patient", "subject":  return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name IN ('patient', 'subject')"
        case "encounter":           return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'encounter'"
        case "performer":           return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'performer'"
        case "based-on":            return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'based-on'"
        case "media":               return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'media'"
        case "result":              return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'result'"
        case "results-interpreter": return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'results-interpreter'"
        case "specimen":            return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'DiagnosticReport' AND param_name = 'specimen'"
        case "status":              return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = 'status'"
        case "code":                return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = 'code'"
        case "category":            return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = 'category'"
        case "conclusion":          return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = 'conclusion'"
        case "identifier":          return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DiagnosticReport' AND param_name = 'identifier'"
        case "date":                return "SELECT DISTINCT resource_id FROM idx_date WHERE resource_type = 'DiagnosticReport' AND param_name = 'date'"
        case "issued":              return "SELECT DISTINCT resource_id FROM idx_date WHERE resource_type = 'DiagnosticReport' AND param_name = 'issued'"
        default:                    return nil
        }
    }
}
