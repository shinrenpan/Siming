...
// ── Inner variants (no BEGIN/COMMIT) — for use inside an existing transaction ─

/// Executes the write SQL and refreshes index rows.
/// Caller is responsible for BEGIN/COMMIT/ROLLBACK on the connection.
/// Throws FHIRServerError.versionConflict when If-Match mismatches.
public func writeResourceInner(
...
/// Executes the delete SQL and clears index rows.
/// Caller is responsible for BEGIN/COMMIT/ROLLBACK on the connection.
/// Returns (versionId, lastUpdated, alreadyDeleted).
/// alreadyDeleted == true → resource was already deleted; treat as idempotent 204.
/// Throws FHIRServerError.notFound / .versionConflict on error.
public func deleteResourceInner(
    conn: PostgresConnection,
    resourceType: String,
    id: String,
    ifMatch: Int64?,
    ...
    for try await lu in insRows.decode(Date.self, context: .default) { lastUpdated = lu }

    _ = try await conn.query("SELECT clear_index_rows(\(resourceType), \(id))", logger: logger)
    return (nextVersion, lastUpdated, false)
}

// ── Outer wrappers (own BEGIN/COMMIT/ROLLBACK) — used by individual stores ───
...
public func writeResource(
    ...
    do {
        let r = try await writeResourceInner(
            conn: conn, resourceType: resourceType, id: id,
            jsonString: jsonString, ifMatch: ifMatch, params: params, logger: logger)
...
public func deleteResource(
    ...
    _ = try await conn.query("BEGIN", logger: logger)
    do {
        let r = try await deleteResourceInner(
            conn: conn, resourceType: resourceType, id: id,
            ifMatch: ifMatch, logger: logger)
        _ = try await conn.query("COMMIT", logger: logger)
        return r
    } catch {
        _ = try? await conn.query("ROLLBACK", logger: logger)
