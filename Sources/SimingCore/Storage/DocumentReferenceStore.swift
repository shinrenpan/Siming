...
public struct DocumentReferenceStore: Sendable {
    ...
    public struct SearchResult: Sendable {
        ...
        public let nextCursor: DocumentReferenceSearchQuery.SearchCursor?
    }
    ...
    public func create(_ docRef: DocumentReference) async throws -> WriteResult {
        let id = UUID().uuidString.lowercased()
        return try await write(id: id, docRef: docRef, ifMatch: nil)
    }

    public func update(id: String, docRef: DocumentReference, ifMatch: Int64?) async throws -> WriteResult {
        return try await write(id: id, docRef: docRef, ifMatch: ifMatch)
    }
        ...
        try await client.withConnection { conn in
            ...
            do {
                let rows = try await conn.query(
                    ...
                    WHERE resource_type = 'DocumentReference' AND id = \(id)
                ...
                guard let current = currentVersion else {
                    ...
                    throw FHIRServerError.notFound(resourceType: "DocumentReference", id: id)
                }
                if isDeleted {
                    _ = try? await conn.query("ROLLBACK", logger: logger)
                ...
                let insRows = try await conn.query(
                    ...
                    VALUES ('DocumentReference', \(id), \(nextVersion), now(), '{}'::jsonb, true)
                    RETURNING last_updated
                    """, logger: logger)
                var lastUpdated = Date()
                for try await (d) in insRows.decode(Date.self, context: .default) { lastUpdated = d }

                _ = try await conn.query("DELETE FROM idx_token     WHERE resource_type = 'DocumentReference' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_string    WHERE resource_type = 'DocumentReference' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_date      WHERE resource_type = 'DocumentReference' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_reference WHERE resource_type = 'DocumentReference' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_quantity  WHERE resource_type = 'DocumentReference' AND resource_id = \(id)", logger: logger)
    ...
    public func vread(id: String, versionId: Int64) async throws -> ReadResult {
        try await client.withConnection { conn in
            let rows = try await conn.query(
                ...
                FROM resources
                WHERE resource_type = 'DocumentReference' AND id = \(id) AND version_id = \(versionId)
                """, logger: logger)
            for try await (vid, lastUpdated, content, deleted) in
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                if deleted { throw FHIRServerError.gone(resourceType: "DocumentReference", id: id) }
    ...
    public func history(id: String) async throws -> [HistoryRawEntry] {
        try await client.withConnection { conn in
                ...
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                ...
                entries.append(HistoryRawEntry(resourceType: "DocumentReference", id: id, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
        ...
        try await client.withConnection { conn in
            ...
            if let since {
                rows = try await conn.query(
                    ...
                    FROM resources
                    WHERE resource_type = 'DocumentReference' AND last_updated >= \(since)
            ...
            {
                let jsonData: Data? = deleted ? nil : injectMeta(into: content, versionId: vid, lastUpdated: lastUpdated)
                entries.append(HistoryRawEntry(resourceType: "DocumentReference", id: rid, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
            return entries
        }
    }
    ...
    public func search(query: DocumentReferenceSearchQuery) async throws -> SearchResult {
        ...
        return try await client.withConnection { conn in
            ...
            let nextCursor: DocumentReferenceSearchQuery.SearchCursor?
            if hasNext, let lastEntry = page.last, let lastSortVal = pageSortVals.last {
                nextCursor = DocumentReferenceSearchQuery.SearchCursor(
    ...
    private func write(id: String, docRef: DocumentReference, ifMatch: Int64?) async throws -> WriteResult {
        try validate(docRef)
        ...
        let searchParams = extractDocumentReferenceSearchParams(resource)
        ...
        return try await client.withConnection { conn in
            ...
            do {
                ...
                let insRows = try await conn.query(
                    ...
                    VALUES ('DocumentReference', \(id), \(nextVersion), now(), \(jsonString)::jsonb, false)
                    RETURNING last_updated
                    """, logger: logger)
                var lastUpdated = Date()
                for try await (d) in insRows.decode(Date.self, context: .default) { lastUpdated = d }
    ...
    private func validate(_ docRef: DocumentReference) throws {}
    ...
    private func replaceIndexRows(conn: PostgresConnection, id: String, params: SearchParams) async throws {
        ...
        for row in params.tokens {
            _ = try await conn.query(
                "INSERT INTO idx_token (resource_type, resource_id, param_name, system, code) VALUES ('DocumentReference', \(id), \(row.paramName), \(row.system), \(row.code))",
                logger: logger)
        }
        for row in params.strings {
            _ = try await conn.query(
                "INSERT INTO idx_string (resource_type, resource_id, param_name, value) VALUES ('DocumentReference', \(id), \(row.paramName), \(row.value))",
                logger: logger)
        }
        for row in params.dates {
            _ = try await conn.query(
                "INSERT INTO idx_date (resource_type, resource_id, param_name, date_start, date_end) VALUES ('DocumentReference', \(id), \(row.paramName), \(row.dateStart), \(row.dateEnd))",
                logger: logger)
        }
        for row in params.references {
            _ = try await conn.query(
                "INSERT INTO idx_reference (resource_type, resource_id, param_name, ref_type, ref_id) VALUES ('DocumentReference', \(id), \(row.paramName), \(row.refType), \(row.refId))",
                logger: logger)
        }
        for row in params.quantities {
            _ = try await conn.query(
                "INSERT INTO idx_quantity (resource_type, resource_id, param_name, system, code, value) VALUES ('DocumentReference', \(id), \(row.paramName), \(row.system), \(row.code), \(row.value))",
                logger: logger)
        }
    }

    private func buildSearchSQL(query: DocumentReferenceSearchQuery) throws -> (String, PostgresBindings) {
        ...
        func tokenORCTE(name: String, paramName: String, tokens: [DocumentReferenceSearchQuery.TokenParam]) -> (String, String) {
        ...
        func tokenNotCondition(paramName: String, tokens: [DocumentReferenceSearchQuery.TokenParam]) -> String {
            var orClauses: [String] = []
            for tok in tokens {
                if tok.code.isEmpty, let sys = tok.system {
                    orClauses.append("system = \(bind(sys))")
        ...
        func dateCTE(name: String, paramName: String, dp: DocumentReferenceSearchQuery.DateParam) -> (String, String) {
        ...
        case .dateAscending, .dateDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_date WHERE resource_type = 'DocumentReference' AND param_name = 'date' " +
        ...
        case .statusAscending, .statusDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_token WHERE resource_type = 'DocumentReference' AND param_name = 'status' " +
        ...
        let fromClause = skipTotal
            ? "FROM paged p JOIN resources r ON r.resource_type = 'DocumentReference' AND r.id = p.id AND r.version_id = p.version_id"
            : "FROM paged p CROSS JOIN total_count t JOIN resources r ON r.resource_type = 'DocumentReference' AND r.id = p.id AND r.version_id = p.version_id"
    ...
    private func documentReferenceMissingSubquery(param: String) -> String? {
        switch param {
        case "status":         return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DocumentReference' AND param_name = 'status'"
        case "type":           return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DocumentReference' AND param_name = 'type'"
        case "category":       return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DocumentReference' AND param_name = 'category'"
        case "identifier":     return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'DocumentReference' AND param_name = 'identifier'"
