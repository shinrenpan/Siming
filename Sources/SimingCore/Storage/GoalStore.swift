...
public struct GoalStore: Sendable {
    ...
    public struct SearchResult: Sendable {
        ...
        public let nextCursor: GoalSearchQuery.SearchCursor?
    }
    ...
    public func create(_ goal: Goal) async throws -> WriteResult {
        let id = UUID().uuidString.lowercased()
        return try await write(id: id, goal: goal, ifMatch: nil)
    }

    public func update(id: String, goal: Goal, ifMatch: Int64?) async throws -> WriteResult {
        return try await write(id: id, goal: goal, ifMatch: ifMatch)
    }
        ...
        try await client.withConnection { conn in
            ...
            do {
                let rows = try await conn.query(
                    ...
                    WHERE resource_type = 'Goal' AND id = \(id)
                ...
                guard let current = currentVersion else {
                    ...
                    throw FHIRServerError.notFound(resourceType: "Goal", id: id)
                }
                if isDeleted {
                    _ = try? await conn.query("ROLLBACK", logger: logger)
                ...
                let insRows = try await conn.query(
                    ...
                    VALUES ('Goal', \(id), \(nextVersion), now(), '{}'::jsonb, true)
                    RETURNING last_updated
                    """, logger: logger)
                var lastUpdated = Date()
                for try await (d) in insRows.decode(Date.self, context: .default) { lastUpdated = d }

                _ = try await conn.query("DELETE FROM idx_token     WHERE resource_type = 'Goal' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_string    WHERE resource_type = 'Goal' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_date      WHERE resource_type = 'Goal' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_reference WHERE resource_type = 'Goal' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_quantity  WHERE resource_type = 'Goal' AND resource_id = \(id)", logger: logger)
    ...
    public func vread(id: String, versionId: Int64) async throws -> ReadResult {
        try await client.withConnection { conn in
            let rows = try await conn.query(
                ...
                FROM resources
                WHERE resource_type = 'Goal' AND id = \(id) AND version_id = \(versionId)
                """, logger: logger)
            for try await (vid, lastUpdated, content, deleted) in
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                if deleted { throw FHIRServerError.gone(resourceType: "Goal", id: id) }
    ...
    public func history(id: String) async throws -> [HistoryRawEntry] {
        try await client.withConnection { conn in
                ...
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                ...
                entries.append(HistoryRawEntry(resourceType: "Goal", id: id, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
        ...
        try await client.withConnection { conn in
            ...
            if let since {
                rows = try await conn.query(
                    ...
                    FROM resources
                    WHERE resource_type = 'Goal' AND last_updated >= \(since)
            ...
            {
                let jsonData: Data? = deleted ? nil : injectMeta(into: content, versionId: vid, lastUpdated: lastUpdated)
                entries.append(HistoryRawEntry(resourceType: "Goal", id: rid, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
            return entries
        }
    }
    ...
    public func search(query: GoalSearchQuery) async throws -> SearchResult {
        if query.count == 0 {
        ...
        return try await client.withConnection { conn in
            ...
            let nextCursor: GoalSearchQuery.SearchCursor?
            if hasNext, let lastEntry = page.last, let lastSortVal = pageSortVals.last {
                nextCursor = GoalSearchQuery.SearchCursor(
    ...
    private func write(id: String, goal: Goal, ifMatch: Int64?) async throws -> WriteResult {
        try validate(goal)
        ...
        let searchParams = extractGoalSearchParams(resource)
        ...
        return try await client.withConnection { conn in
            ...
            do {
                ...
                let insRows = try await conn.query(
                    ...
                    VALUES ('Goal', \(id), \(nextVersion), now(), \(jsonString)::jsonb, false)
                    RETURNING last_updated
                    """, logger: logger)
                var lastUpdated = Date()
                for try await (d) in insRows.decode(Date.self, context: .default) { lastUpdated = d }
    ...
    private func validate(_ goal: Goal) throws {}
    ...
    private func replaceIndexRows(conn: PostgresConnection, id: String, params: SearchParams) async throws {
        ...
        for row in params.tokens {
            _ = try await conn.query(
                "INSERT INTO idx_token (resource_type, resource_id, param_name, system, code) VALUES ('Goal', \(id), \(row.paramName), \(row.system), \(row.code))",
                logger: logger)
        }
        for row in params.strings {
            _ = try await conn.query(
                "INSERT INTO idx_string (resource_type, resource_id, param_name, value) VALUES ('Goal', \(id), \(row.paramName), \(row.value))",
                logger: logger)
        }
        for row in params.dates {
            _ = try await conn.query(
                "INSERT INTO idx_date (resource_type, resource_id, param_name, date_start, date_end) VALUES ('Goal', \(id), \(row.paramName), \(row.dateStart), \(row.dateEnd))",
                logger: logger)
        }
        for row in params.references {
            _ = try await conn.query(
                "INSERT INTO idx_reference (resource_type, resource_id, param_name, ref_type, ref_id) VALUES ('Goal', \(id), \(row.paramName), \(row.refType), \(row.refId))",
                logger: logger)
        }
        for row in params.quantities {
            _ = try await conn.query(
                "INSERT INTO idx_quantity (resource_type, resource_id, param_name, system, code, value) VALUES ('Goal', \(id), \(row.paramName), \(row.system), \(row.code), \(row.value))",
                logger: logger)
        }
    }

    private func buildSearchSQL(query: GoalSearchQuery) throws -> (String, PostgresBindings) {
        ...
        func tokenORCTE(name: String, paramName: String, tokens: [GoalSearchQuery.TokenParam]) -> (String, String) {
            ...
            return (name, "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'Goal' AND param_name = '\(paramName)' AND (\(orClauses.joined(separator: " OR ")))")
        }

        func tokenNotCondition(paramName: String, tokens: [GoalSearchQuery.TokenParam]) -> String {
            var orClauses: [String] = []
            for tok in tokens {
                if tok.code.isEmpty, let sys = tok.system {
                    orClauses.append("system = \(bind(sys))")
        ...
        func refCTE(name: String, paramName: String, ref: String) -> (String, String) {
            ...
            if parts.count == 2 {
                ...
                return (name, "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'Goal' AND param_name = '\(paramName)' AND ref_type = \(refTypeP) AND ref_id = \(refIdP)")
            } else {
                let refIdP = bind(ref)
                return (name, "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'Goal' AND param_name = '\(paramName)' AND ref_id = \(refIdP)")
            }
        }

        func dateCTE(name: String, paramName: String, dp: GoalSearchQuery.DateParam) -> (String, String) {
        ...
        // token CTEs
        if !query.lifecycleStatus.isEmpty   { filterCTEs.append(tokenORCTE(name: "f_lstatus", paramName: "lifecycle-status",    tokens: query.lifecycleStatus)) }
        if !query.achievementStatus.isEmpty { filterCTEs.append(tokenORCTE(name: "f_astatus", paramName: "achievement-status", tokens: query.achievementStatus)) }
        ...
        if !query.identifier.isEmpty {
            ...
            if !orClauses.isEmpty {
                filterCTEs.append(("f_ident", "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'Goal' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR ")))"))
            }
        }

        // date CTEs
        for (i, dp) in query.startDate.enumerated() {
            filterCTEs.append(dateCTE(name: "f_sdate\(i)", paramName: "start-date", dp: dp))
        }
        for (i, dp) in query.targetDate.enumerated() {
            filterCTEs.append(dateCTE(name: "f_tdate\(i)", paramName: "target-date", dp: dp))
        }
        ...
        for (i, chain) in query.chains.enumerated() {
            if let (name, sql) = chainFilterCTE(
                index: filterCTEs.count + i, sourceType: "Goal",
        ...
        case .statusAscending, .statusDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_token WHERE resource_type = 'Goal' AND param_name = 'lifecycle-status' " +
        ...
        let fromClause = skipTotal
            ? "FROM paged p JOIN resources r ON r.resource_type = 'Goal' AND r.id = p.id AND r.version_id = p.version_id"
            : "FROM paged p CROSS JOIN total_count t JOIN resources r ON r.resource_type = 'Goal' AND r.id = p.id AND r.version_id = p.version_id"

        let sql = "\(withClause)\nSELECT p.id, p.version_id, p.last_updated, r.content, \(totalExpr), \(finalSortValSQL) AS sort_val_text\n\(fromClause)"
        return (sql, binds)
    }

    private func buildCountSQL(query: GoalSearchQuery) throws -> (String, PostgresBindings) {
        var binds = PostgresBindings()
        var n = 0
        func bind(_ val: some PostgresDynamicTypeEncodable) -> String {
            n += 1; binds.append(val); return "$\(n)"
        ...
        func cTokenORCTE(name: String, paramName: String, tokens: [GoalSearchQuery.TokenParam]) -> (String, String) {
            var orClauses: [String] = []
            for tok in tokens {
                if tok.code.isEmpty, let sys = tok.system {
                    orClauses.append("system = \(bind(sys))")
        ...
        func cDateCTE(name: String, paramName: String, dp: GoalSearchQuery.DateParam) -> (String, String) {
            let startP = bind(dp.dateStart); let endP = bind(dp.dateEnd)
            let cond: String
            switch dp.prefix {
            case .eq: cond = "date_start >= \(startP) AND date_end <= \(endP)"
        ...
        if !query.lifecycleStatus.isEmpty   { filterCTEs.append(cTokenORCTE(name: "f_lstatus",  paramName: "lifecycle-status",   tokens: query.lifecycleStatus)) }
        if !query.achievementStatus.isEmpty { filterCTEs.append(cTokenORCTE(name: "f_astatus",  paramName: "achievement-status", tokens: query.achievementStatus)) }
        ...
        for (i, dp) in query.startDate.enumerated() {
            filterCTEs.append(cDateCTE(name: "f_sdate\(i)", paramName: "start-date", dp: dp))
        }
        for (i, dp) in query.targetDate.enumerated() {
            filterCTEs.append(cDateCTE(name: "f_tdate\(i)", paramName: "target-date", dp: dp))
        }
    ...
    private func goalMissingSubquery(param: String) -> String? {
        switch param {
        case "lifecycle-status":   return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'Goal' AND param_name = 'lifecycle-status'"
        case "achievement-status": return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'Goal' AND param_name = 'achievement-status'"
        case "category":           return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'Goal' AND param_name = 'category'"
        case "identifier":         return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'Goal' AND param_name = 'identifier'"
        case "start-date":         return "SELECT DISTINCT resource_id FROM idx_date WHERE resource_type = 'Goal' AND param_name = 'start-date'"
        case "target-date":        return "SELECT DISTINCT resource_id FROM idx_date WHERE resource_type = 'Goal' AND param_name = 'target-date'"
        case "subject":            return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'Goal' AND param_name = 'subject'"
        case "patient":            return "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'Goal' AND param_name = 'patient'"
        default:                   return nil
        }
    }
}
