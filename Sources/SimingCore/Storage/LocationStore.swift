...
public struct LocationStore: Sendable {
    ...
    private func write(id: String, location: Location, ifMatch: Int64?) async throws -> WriteResult {
        ...
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "Location", json: _jsonObj, terminology: terminology)
        }
    ...
    private func buildSearchSQL(query: LocationSearchQuery) throws -> (String, PostgresBindings) {
        ...
        if !query.statusNot.isEmpty {
            extraConditions.append(tokenNotCondition(paramName: "status", tokens: query.statusNot))
        }
        if !query.typeNot.isEmpty {
            extraConditions.append(tokenNotCondition(paramName: "type", tokens: query.typeNot))
        }
        if !query.operationalStatusNot.isEmpty {
            extraConditions.append(tokenNotCondition(paramName: "operational-status", tokens: query.operationalStatusNot))
        }
        ...
        if !query.identifierNot.isEmpty {
            ...
            if !orClauses.isEmpty {
                extraConditions.append("r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'Location' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR "))))")
            }
        }
        ...
        for (i, tt) in query.tokenTexts.enumerated() {
            ...
            filterCTEs.append(("f_ttext\(i)",
                "SELECT DISTINCT resource_id FROM idx_string WHERE resource_type = 'Location' AND param_name = \(pn) AND value ILIKE \(val)"))
        }
