...
public struct MedicationStatementStore: Sendable {
    ...
    public func delete(id: String, ifMatch: Int64?) async throws -> DeleteResult {
        try await client.withConnection { conn in
            let (versionId, lastUpdated, _) = try await deleteResource(
                conn: conn, resourceType: "MedicationStatement", id: id, ifMatch: ifMatch, logger: logger)
        ...
        try await client.withConnection { conn in
            ...
            if let since {
                rows = try await conn.query(
                    ...
                    SELECT version_id, last_updated, content, deleted
                    FROM resources
                    WHERE resource_type = 'MedicationStatement' AND id = \(id) AND last_updated >= \(since)
            ...
            if entries.isEmpty {
                if since != nil {
                    let existRows = try await conn.query(
                        """
                        SELECT 1 FROM resources WHERE resource_type = 'MedicationStatement' AND id = \(id) LIMIT 1
                        """, logger: logger)
                    var exists = false
                    for try await _ in existRows { exists = true }
                    if !exists { throw FHIRServerError.notFound(resourceType: "MedicationStatement", id: id) }
                } else {
                    throw FHIRServerError.notFound(resourceType: "MedicationStatement", id: id)
                }
            }
    ...
    private func write(id: String, medicationStatement: MedicationStatement, ifMatch: Int64?) async throws -> WriteResult {
        ...
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "MedicationStatement", json: _jsonObj, terminology: terminology)
        }
        let jsonString = String(data: jsonData, encoding: .utf8)!
        var searchParams = extractMedicationStatementSearchParams(ms)
        appendMetaParams(&searchParams, meta: medicationStatement.meta)

        return try await client.withConnection { conn in
            let (versionId, lastUpdated) = try await writeResource(
                conn: conn, resourceType: "MedicationStatement", id: id,
    ...
    private func buildSearchSQL(query: MedicationStatementSearchQuery) throws -> (String, PostgresBindings) {
        ...
        for (i, tt) in query.tokenTexts.enumerated() {
            ...
            filterCTEs.append(("f_ttext\(i)",
                "SELECT DISTINCT resource_id FROM idx_string WHERE resource_type = 'MedicationStatement' AND param_name = \(pn) AND value ILIKE \(val)"))
        }
    ...
    private func buildCountSQL(query: MedicationStatementSearchQuery) throws -> (String, PostgresBindings) {
        ...
        let idsInner = buildCountIdsInner(
            resourceType: "MedicationStatement", filterCTEs: filterCTEs, whereConditions: whereConditions)
