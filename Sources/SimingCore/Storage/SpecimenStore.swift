...
public struct SpecimenStore: Sendable {
    ...
    private func write(id: String, specimen: Specimen, ifMatch: Int64?) async throws -> WriteResult {
        ...
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "Specimen", json: _jsonObj, terminology: terminology)
        }
        let jsonString = String(data: jsonData, encoding: .utf8)!
        var searchParams = extractSpecimenSearchParams(resource)
        appendMetaParams(&searchParams, meta: specimen.meta)
    ...
    private func buildSearchSQL(query: SpecimenSearchQuery) throws -> (String, PostgresBindings) {
        ...
        for (i, tt) in query.tokenTexts.enumerated() {
            ...
            filterCTEs.append(("f_ttext\(i)",
                "SELECT DISTINCT resource_id FROM idx_string WHERE resource_type = 'Specimen' AND param_name = \(pn) AND value ILIKE \(val)"))
        }
