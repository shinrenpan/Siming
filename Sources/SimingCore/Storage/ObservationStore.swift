...
public struct ObservationStore: Sendable {
    ...
    private func buildSearchSQL(query: ObservationSearchQuery) throws -> (String, PostgresBindings) {
        ...
        for (i, tt) in query.tokenTexts.enumerated() {
            ...
            filterCTEs.append(("f_ttext\(i)",
                "SELECT DISTINCT resource_id FROM idx_string WHERE resource_type = 'Observation' AND param_name = \(pn) AND value ILIKE \(val)"))
        }
    ...
    private func buildCountSQL(query: ObservationSearchQuery) throws -> (String, PostgresBindings) {
        ...
        let idsInner = buildCountIdsInner(
            resourceType: "Observation", filterCTEs: filterCTEs, whereConditions: whereConditions)
