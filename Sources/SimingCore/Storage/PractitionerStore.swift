...
public struct PractitionerStore: Sendable {
    ...
    private func write(id: String, practitioner: Practitioner, ifMatch: Int64?) async throws -> WriteResult {
        ...
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "Practitioner", json: _jsonObj, terminology: terminology)
        }
    ...
    private func buildSearchSQL(query: PractitionerSearchQuery) throws -> (String, PostgresBindings) {
        ...
        if !query.genderNot.isEmpty {
            ...
            extraConditions.append("r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'Practitioner' AND param_name = 'gender' AND code IN (\(phs)))")
        }
        ...
        if !query.communicationNot.isEmpty {
            ...
            extraConditions.append("r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'Practitioner' AND param_name = 'communication' AND (\(orClauses.joined(separator: " OR "))))")
        }
        ...
        if !query.identifierNot.isEmpty {
            ...
            if !orClauses.isEmpty {
                extraConditions.append("r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'Practitioner' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR "))))")
            }
        }
        ...
        for (i, tt) in query.tokenTexts.enumerated() {
            ...
            filterCTEs.append(("f_ttext\(i)",
                "SELECT DISTINCT resource_id FROM idx_string WHERE resource_type = 'Practitioner' AND param_name = \(pn) AND value ILIKE \(val)"))
        }
    ...
    private func buildCountSQL(query: PractitionerSearchQuery) throws -> (String, PostgresBindings) {
        ...
        if !query.identifierNot.isEmpty {
            ...
            if !orClauses.isEmpty {
                whereConditions.append("r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'Practitioner' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR "))))")
            }
        }
        ...
        let idsInner = buildCountIdsInner(
            resourceType: "Practitioner", filterCTEs: filterCTEs, whereConditions: whereConditions)
