...
public struct MedicationRequestStore: Sendable {
    ...
    private func write(id: String, medicationRequest: MedicationRequest, ifMatch: Int64?) async throws -> WriteResult {
        ...
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "MedicationRequest", json: _jsonObj, terminology: terminology)
        }
    ...
    private func buildSearchSQL(query: MedicationRequestSearchQuery) throws -> (String, PostgresBindings) {
        ...
        if !query.identifierNot.isEmpty {
            ...
            if !orClauses.isEmpty {
                extraConditions.append("r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'MedicationRequest' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR "))))")
            }
        }

        if !query.statusNot.isEmpty   { extraConditions.append(notTokenCond(paramName: "status",   tokens: query.statusNot)) }
        if !query.intentNot.isEmpty   { extraConditions.append(notTokenCond(paramName: "intent",   tokens: query.intentNot)) }
        ...
        if !query.priorityNot.isEmpty { extraConditions.append(notTokenCond(paramName: "priority", tokens: query.priorityNot)) }
        if !query.intendedPerformerTypeNot.isEmpty { extraConditions.append(notTokenCond(paramName: "intended-performertype", tokens: query.intendedPerformerTypeNot)) }
        ...
        for (i, tt) in query.tokenTexts.enumerated() {
            ...
            filterCTEs.append(("f_ttext\(i)",
                "SELECT DISTINCT resource_id FROM idx_string WHERE resource_type = 'MedicationRequest' AND param_name = \(pn) AND value ILIKE \(val)"))
        }
    ...
    private func buildCountSQL(query: MedicationRequestSearchQuery) throws -> (String, PostgresBindings) {
        ...
        if !query.identifierNot.isEmpty {
            ...
            if !orClauses.isEmpty {
                whereConditions.append("r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'MedicationRequest' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR "))))")
            }
        }
        ...
        let idsInner = buildCountIdsInner(
            resourceType: "MedicationRequest", filterCTEs: filterCTEs, whereConditions: whereConditions)
