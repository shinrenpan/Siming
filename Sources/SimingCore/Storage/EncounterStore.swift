...
public struct EncounterStore: Sendable {
    ...
    private func write(id: String, encounter: Encounter, ifMatch: Int64?) async throws -> WriteResult {
        ...
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "Encounter", json: _jsonObj, terminology: terminology)
        }
    ...
    private func buildSearchSQL(query: EncounterSearchQuery) throws -> (String, PostgresBindings) {
        ...
        for (i, tt) in query.tokenTexts.enumerated() {
            ...
            filterCTEs.append(("f_ttext\(i)",
                "SELECT DISTINCT resource_id FROM idx_string WHERE resource_type = 'Encounter' AND param_name = \(pn) AND value ILIKE \(val)"))
        }
    ...
    private func buildCountSQL(query: EncounterSearchQuery) throws -> (String, PostgresBindings) {
        ...
        let idsInner = buildCountIdsInner(
            resourceType: "Encounter", filterCTEs: filterCTEs, whereConditions: whereConditions)
