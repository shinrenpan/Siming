...
public struct AllergyIntoleranceStore: Sendable {
    ...
    public struct SearchResult: Sendable {
        ...
        public let nextCursor: AllergyIntoleranceSearchQuery.SearchCursor?
    }
    ...
    public func create(_ ai: AllergyIntolerance) async throws -> WriteResult {
        let id = UUID().uuidString.lowercased()
        return try await write(id: id, allergyIntolerance: ai, ifMatch: nil)
    }

    public func update(id: String, allergyIntolerance: AllergyIntolerance, ifMatch: Int64?) async throws -> WriteResult {
        return try await write(id: id, allergyIntolerance: allergyIntolerance, ifMatch: ifMatch)
    }
        ...
        try await client.withConnection { conn in
            ...
            do {
                let rows = try await conn.query(
                    ...
                    WHERE resource_type = 'AllergyIntolerance' AND id = \(id)
                ...
                guard let current = currentVersion else {
                    ...
                    throw FHIRServerError.notFound(resourceType: "AllergyIntolerance", id: id)
                }
                if isDeleted {
                    _ = try? await conn.query("ROLLBACK", logger: logger)
                ...
                let insRows = try await conn.query(
                    ...
                    VALUES ('AllergyIntolerance', \(id), \(nextVersion), now(), '{}'::jsonb, true)
                    RETURNING last_updated
                    """, logger: logger)
                var lastUpdated = Date()
                for try await (d) in insRows.decode(Date.self, context: .default) { lastUpdated = d }

                _ = try await conn.query("DELETE FROM idx_token     WHERE resource_type = 'AllergyIntolerance' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_string    WHERE resource_type = 'AllergyIntolerance' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_date      WHERE resource_type = 'AllergyIntolerance' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_reference WHERE resource_type = 'AllergyIntolerance' AND resource_id = \(id)", logger: logger)
                _ = try await conn.query("DELETE FROM idx_quantity  WHERE resource_type = 'AllergyIntolerance' AND resource_id = \(id)", logger: logger)
    ...
    public func vread(id: String, versionId: Int64) async throws -> ReadResult {
        try await client.withConnection { conn in
            let rows = try await conn.query(
                ...
                FROM resources
                WHERE resource_type = 'AllergyIntolerance' AND id = \(id) AND version_id = \(versionId)
                """, logger: logger)
            for try await (vid, lastUpdated, content, deleted) in
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                if deleted { throw FHIRServerError.gone(resourceType: "AllergyIntolerance", id: id) }
    ...
    public func history(id: String) async throws -> [HistoryRawEntry] {
        try await client.withConnection { conn in
                ...
                rows.decode((Int64, Date, String, Bool).self, context: .default)
            {
                ...
                entries.append(HistoryRawEntry(resourceType: "AllergyIntolerance", id: id, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
        ...
        try await client.withConnection { conn in
            ...
            if let since {
                rows = try await conn.query(
                    ...
                    FROM resources
                    WHERE resource_type = 'AllergyIntolerance' AND last_updated >= \(since)
            ...
            {
                let jsonData: Data? = deleted ? nil : injectMeta(into: content, versionId: vid, lastUpdated: lastUpdated)
                entries.append(HistoryRawEntry(resourceType: "AllergyIntolerance", id: rid, versionId: vid, lastUpdated: lastUpdated, jsonData: jsonData, deleted: deleted))
            }
            return entries
        }
    }
    ...
    public func search(query: AllergyIntoleranceSearchQuery) async throws -> SearchResult {
        if query.count == 0 {
        ...
        return try await client.withConnection { conn in
            ...
            let nextCursor: AllergyIntoleranceSearchQuery.SearchCursor?
            if hasNext, let lastEntry = page.last, let lastSortVal = pageSortVals.last {
                nextCursor = AllergyIntoleranceSearchQuery.SearchCursor(
    ...
    private func write(id: String, allergyIntolerance: AllergyIntolerance, ifMatch: Int64?) async throws -> WriteResult {
        // Validation hook — keep this call; it's one of the three open doors.
        try validate(allergyIntolerance)
        ...
        return try await client.withConnection { conn in
            ...
            do {
                ...
                let insRows = try await conn.query(
                    ...
                    VALUES ('AllergyIntolerance', \(id), \(nextVersion), now(), \(jsonString)::jsonb, false)
                    RETURNING last_updated
                    """, logger: logger)
                var lastUpdated = Date()
                for try await (d) in insRows.decode(Date.self, context: .default) { lastUpdated = d }
    ...
    private func validate(_ ai: AllergyIntolerance) throws {}
    ...
    private func replaceIndexRows(conn: PostgresConnection, id: String, params: SearchParams) async throws {
        ...
        for row in params.tokens {
            _ = try await conn.query(
                "INSERT INTO idx_token (resource_type, resource_id, param_name, system, code) VALUES ('AllergyIntolerance', \(id), \(row.paramName), \(row.system), \(row.code))",
                logger: logger)
        }
        for row in params.strings {
            _ = try await conn.query(
                "INSERT INTO idx_string (resource_type, resource_id, param_name, value) VALUES ('AllergyIntolerance', \(id), \(row.paramName), \(row.value))",
                logger: logger)
        }
        for row in params.dates {
            _ = try await conn.query(
                "INSERT INTO idx_date (resource_type, resource_id, param_name, date_start, date_end) VALUES ('AllergyIntolerance', \(id), \(row.paramName), \(row.dateStart), \(row.dateEnd))",
                logger: logger)
        }
        for row in params.references {
            _ = try await conn.query(
                "INSERT INTO idx_reference (resource_type, resource_id, param_name, ref_type, ref_id) VALUES ('AllergyIntolerance', \(id), \(row.paramName), \(row.refType), \(row.refId))",
                logger: logger)
        }
        for row in params.quantities {
            _ = try await conn.query(
                "INSERT INTO idx_quantity (resource_type, resource_id, param_name, system, code, value) VALUES ('AllergyIntolerance', \(id), \(row.paramName), \(row.system), \(row.code), \(row.value))",
                logger: logger)
        }
    }

    private func buildSearchSQL(query: AllergyIntoleranceSearchQuery) throws -> (String, PostgresBindings) {
        ...
        if let subject = query.subject {
            ...
            if parts.count == 2 {
                ...
                filterCTEs.append(("f_subject", """
                    ...
                    WHERE resource_type = 'AllergyIntolerance' AND param_name = 'patient'
        ...
        func tokenORCTE(name: String, paramName: String, tokens: [AllergyIntoleranceSearchQuery.TokenParam]) -> (String, String) {
        ...
        if !query.identifier.isEmpty {
            ...
            if !orClauses.isEmpty {
                filterCTEs.append(("f_ident",
                    "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'AllergyIntolerance' AND param_name = 'identifier' AND (\(orClauses.joined(separator: " OR ")))"))
            }
        }
        ...
        func notTokenCond(paramName: String, tokens: [AllergyIntoleranceSearchQuery.TokenParam]) -> String {
            ...
            return "r.id NOT IN (SELECT resource_id FROM idx_token WHERE resource_type = 'AllergyIntolerance' AND param_name = '\(paramName)' AND (\(orClauses.joined(separator: " OR "))))"
        }
        ...
        case .dateAscending, .dateDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_date WHERE resource_type = 'AllergyIntolerance' AND param_name = 'date' " +
        ...
        case .clinicalStatusAscending, .clinicalStatusDescending:
            sortKeysCTE = ("sort_keys",
                "SELECT DISTINCT ON (resource_id) resource_id, code AS sv " +
                "FROM idx_token WHERE resource_type = 'AllergyIntolerance' AND param_name = 'clinical-status' " +
        ...
        case .codeAscending, .codeDescending:
            sortKeysCTE = ("sort_keys",
                "SELECT DISTINCT ON (resource_id) resource_id, code AS sv " +
                "FROM idx_token WHERE resource_type = 'AllergyIntolerance' AND param_name = 'code' " +
                "ORDER BY resource_id, code ASC")
            if let cursor = query.cursor {
                let codeP = bind(cursor.sortValue)
                let idP = bind(cursor.id)
        ...
        let fromClause = skipTotal
            ? "FROM paged p JOIN resources r ON r.resource_type = 'AllergyIntolerance' AND r.id = p.id AND r.version_id = p.version_id"
            : "FROM paged p CROSS JOIN total_count t JOIN resources r ON r.resource_type = 'AllergyIntolerance' AND r.id = p.id AND r.version_id = p.version_id"

        let sql = "\(withClause)\nSELECT p.id, p.version_id, p.last_updated, r.content, \(totalExpr), \(finalSortValSQL) AS sort_val_text\n\(fromClause)"
        return (sql, binds)
    }

    private func buildCountSQL(query: AllergyIntoleranceSearchQuery) throws -> (String, PostgresBindings) {
        var binds = PostgresBindings()
        var n = 0
        func bind(_ val: some PostgresDynamicTypeEncodable) -> String {
            n += 1; binds.append(val); return "$\(n)"
        ...
        if let subject = query.subject {
            ...
            if parts.count == 2 {
                let refTypeP = bind(String(parts[0])); let refIdP = bind(String(parts[1]))
                filterCTEs.append(("f_subject",
                    "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'AllergyIntolerance' AND param_name = 'patient' AND ref_type = \(refTypeP) AND ref_id = \(refIdP)"))
            } else {
                let refIdP = bind(subject)
                filterCTEs.append(("f_subject",
                    "SELECT DISTINCT resource_id FROM idx_reference WHERE resource_type = 'AllergyIntolerance' AND param_name = 'patient' AND ref_id = \(refIdP)"))
            }
        }
        ...
        func tokenCTE(name: String, paramName: String, tokens: [AllergyIntoleranceSearchQuery.TokenParam]) -> (String, String) {
            ...
            return (name, "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'AllergyIntolerance' AND param_name = '\(paramName)' AND (\(orClauses.joined(separator: " OR ")))")
        }
        ...
        if !query.type.isEmpty             { filterCTEs.append(tokenCTE(name: "f_type",          paramName: "type",                 tokens: query.type)) }
    ...
    private func allergyIntoleranceMissingSubquery(param: String) -> String? {
        ...
        case "category":              return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'AllergyIntolerance' AND param_name = 'category'"
        case "criticality":           return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'AllergyIntolerance' AND param_name = 'criticality'"
        case "code":                  return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'AllergyIntolerance' AND param_name = 'code'"
        case "identifier":            return "SELECT DISTINCT resource_id FROM idx_token WHERE resource_type = 'AllergyIntolerance' AND param_name = 'identifier'"
        case "date":                  return "SELECT DISTINCT resource_id FROM idx_date WHERE resource_type = 'AllergyIntolerance' AND param_name = 'date'"
        default:                      return nil
        }
    }
}
