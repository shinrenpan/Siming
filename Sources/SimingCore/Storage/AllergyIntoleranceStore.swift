    ...
    public func delete(id: String, ifMatch: Int64?) async throws -> DeleteResult {
        try await client.withConnection { conn in
            let (versionId, lastUpdated, _) = try await deleteResource(
                conn: conn, resourceType: "AllergyIntolerance", id: id, ifMatch: ifMatch, logger: logger)
        ...
        try await client.withConnection { conn in
            ...
            if let since {
                rows = try await conn.query(
                    ...
                    SELECT version_id, last_updated, content, deleted
                    FROM resources
                    WHERE resource_type = 'AllergyIntolerance' AND id = \(id) AND last_updated >= \(since)
            ...
            if entries.isEmpty {
                if since != nil {
                    let existRows = try await conn.query(
                        """
                        SELECT 1 FROM resources WHERE resource_type = 'AllergyIntolerance' AND id = \(id) LIMIT 1
                        """, logger: logger)
                    var exists = false
                    for try await _ in existRows { exists = true }
                    if !exists { throw FHIRServerError.notFound(resourceType: "AllergyIntolerance", id: id) }
                } else {
                    throw FHIRServerError.notFound(resourceType: "AllergyIntolerance", id: id)
                }
            }
    ...
    private func write(id: String, allergyIntolerance: AllergyIntolerance, ifMatch: Int64?) async throws -> WriteResult {
        ...
        return try await client.withConnection { conn in
            let (versionId, lastUpdated) = try await writeResource(
                conn: conn, resourceType: "AllergyIntolerance", id: id,
