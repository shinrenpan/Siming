...
public struct MedicationStore: Sendable {
    ...
    private func write(id: String, medication: Medication, ifMatch: Int64?) async throws -> WriteResult {
        ...
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "Medication", json: _jsonObj, terminology: terminology)
        }
    ...
    private func buildCountSQL(query: MedicationSearchQuery) throws -> (String, PostgresBindings) {
        ...
        let idsInner = buildCountIdsInner(
            resourceType: "Medication", filterCTEs: filterCTEs, whereConditions: whereConditions)
