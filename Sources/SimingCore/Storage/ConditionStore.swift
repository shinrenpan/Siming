...
public struct ConditionStore: Sendable {
    ...
    private func write(id: String, condition: Condition, ifMatch: Int64?) async throws -> WriteResult {
        ...
        if let _jsonObj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] {
            try validateCodes(resourceType: "Condition", json: _jsonObj, terminology: terminology)
        }
    ...
    private func buildCountSQL(query: ConditionSearchQuery) throws -> (String, PostgresBindings) {
        ...
        let idsInner = buildCountIdsInner(
            resourceType: "Condition", filterCTEs: filterCTEs, whereConditions: whereConditions)
