    ...
    private func write(id: String, condition: Condition, ifMatch: Int64?) async throws -> WriteResult {
        ...
        return try await client.withConnection { conn in
            ...
            do {
                ...
                try await replaceIndexRows(conn: conn, resourceType: "Condition", id: id, params: searchParams, logger: logger)
    ...
    private func buildSearchSQL(query: ConditionSearchQuery) throws -> (String, PostgresBindings) {
        ...
        case .clinicalStatusAscending, .clinicalStatusDescending:
            sortKeysCTE = ("sort_keys",
                ...
                "FROM idx_token WHERE resource_type = 'Condition' AND param_name = 'clinical-status' " +
        ...
        case .codeAscending, .codeDescending:
            sortKeysCTE = ("sort_keys",
                "SELECT DISTINCT ON (resource_id) resource_id, code AS sv " +
                "FROM idx_token WHERE resource_type = 'Condition' AND param_name = 'code' " +
                "ORDER BY resource_id, code ASC")
            if let cursor = query.cursor {
                let codeP = bind(cursor.sortValue)
                let idP = bind(cursor.id)
