...
- **Ecosystem:** Jujing (`~/Documents/github/Jujing`) is the clinic staff web UI (React); FHIRpass is the patient iOS app. Both connect via standard FHIR API. Currently backed by HAPI FHIR; migration to Siming planned when MedicationDispense + PractitionerRole + Composition are added.
